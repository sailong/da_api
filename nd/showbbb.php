...
function Getcss($cave_, $par) {
	...
	if ($cave_ == - 1) {
		...
		switch ($option) {
			...
			//低于标准杆1杆：字白底红字是小鸟
			case - 1 :
				$dataInfo = " style=\"text-align:center;color:#ffffff;background:#ed3f2d\"";
				break;
...
<table width="98%" border="0" align="center"  cellpadding="0" cellspacing="0" > 
  ...
  $t=1;
	...
	$sql = "select *,(cave_1+cave_2+cave_3+cave_4+cave_5+cave_6+cave_7+cave_8+cave_9) as lout,(cave_10+cave_11+cave_12+cave_13+cave_14+cave_15+cave_16+cave_17 +cave_18) as lin  from tbl_baofen where 1=1 and  fenzhan_id='".$fenzhan_id."'   ORDER BY $orderby  status desc,total_ju_par,total_score,fenzu_id "; 
  $jdt=DB::query($sql);  
	...
	?>
     ...
     <tr <?php if (($i+1)>($pg1-$pg)&&($i+1)<=$pg1){?>style="display:;" <?php }else{?>style="display:none;"<?php }?> id="<?php echo $i;?>">
      ...
      <?php echo $lib[$i]['order'];?></td>
       ...
       <td class="lst"><?php if($lib[$i]['total_score']>200||$lib[$i]['total_score']==0){echo "-";}else{echo $lib[$i]['total_score'];}?></td>
     </tr>

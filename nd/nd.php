...
if ($uid > 0) {
 ...
 $bf = DB::query ( "select *,(cave_1+cave_2+cave_3+cave_4+cave_5+cave_6+cave_7+cave_8+cave_9) as lout,(cave_10+cave_11+cave_12+cave_13+cave_14+cave_15+cave_16+cave_17 +cave_18) as lin  from " . DB::table ( 'golf_nd_baofen' ) . " where  onlymark= 1271827009  ORDER BY   isend desc,avcave,lin"); 
}
...
$sql="select uid,realname,g_team_name, g_team_id,avcave,tlcave from pre_golf_nd_baofen where onlymark= ".$onlymark." and avcave<999 order by tlcave ";
	 ...
	 $sql="select uid,realname,g_team_name, g_team_id,avcave,tlcave from pre_golf_nd_baofen where onlymark= 1271827009 and avcave<999 order by tlcave ";}

...
//获取赛事ID
$score_info=DB::fetch_first("select sid from ".DB::table("golf_nd_baofen")." where nd_id='".$nd_id."'  ");
...
$uid = ! empty ( $_GET ['uid'] ) ? $_GET ['uid'] : 0;
 ...
 $bf = DB::query ( "select *,(cave_1+cave_2+cave_3+cave_4+cave_5+cave_6+cave_7+cave_8+cave_9) as lout,(cave_10+cave_11+cave_12+cave_13+cave_14+cave_15+cave_16+cave_17 +cave_18) as lin  from " . DB::table ( 'golf_nd_baofen' ) . " where  fenz_id= $fz_id  ORDER BY   isend desc,avcave,lin"); 
...
function Getteamname($team_id)
{
	...
	include_once template("diy:nd/fenzhan"); 

...
class tongjiAction extends field_publicAction
{
	...
	public function reg()
	{
		...
		$reg_all_num=M()->query("select count(uid) as num from pre_common_member_profile where reg_source='".$_SESSION['field_uid']."' limit 1 ");
		...
		$login_yestoday_num=M()->query("select uid from tbl_app_log where field_uid='".$_SESSION['field_uid']."' and app_log_mod='login' and app_log_addtime<'".$today_e."' and app_log_addtime>'".$today_s."' and (user_agent='iPhone' or user_agent='Android') and uid>0 group by uid ");
		$login_yestoday_num_iphone=M()->query("select uid from tbl_app_log where field_uid='".$_SESSION['field_uid']."' and app_log_mod='login' and app_log_addtime<'".$today_e."' and app_log_addtime>'".$today_s."' and user_agent='iPhone' and uid>0 group by uid ");
		$login_yestoday_num_android=M()->query("select uid from tbl_app_log where field_uid='".$_SESSION['field_uid']."' and app_log_mod='login' and app_log_addtime<'".$today_e."' and app_log_addtime>'".$today_s."' and user_agent='Android' and uid>0 group by uid ");
		
		//今天
		$today_s=strtotime(date("Y-m-d",time())." 00:00:00");
		$today_e=strtotime(date("Y-m-d",time())." 23:59:59");
		$login_today_num=M()->query("select uid from tbl_app_log where field_uid='".$_SESSION['field_uid']."' and app_log_mod='login' and app_log_addtime<'".$today_e."' and app_log_addtime>'".$today_s."' and (user_agent='iPhone' or user_agent='Android') and uid>0 group by uid ");
		$login_today_num_iphone=M()->query("select uid from tbl_app_log where field_uid='".$_SESSION['field_uid']."' and app_log_mod='login' and app_log_addtime<'".$today_e."' and app_log_addtime>'".$today_s."' and user_agent='iPhone' and uid>0 group by uid ");
		$login_today_num_android=M()->query("select uid from tbl_app_log where field_uid='".$_SESSION['field_uid']."' and app_log_mod='login' and app_log_addtime<'".$today_e."' and app_log_addtime>'".$today_s."' and user_agent='Android' and uid>0 group by uid ");
		
		$login_all_num=M()->query("select uid from tbl_app_log where field_uid='".$_SESSION['field_uid']."' and app_log_mod='login' and uid>0 group by uid ");
  ...
  <tr>
    <td colspan="3" style="line-height:50px; text-align:center; font-size:18px; font-weight:bold;" bgcolor="#FFFFFF">登录注册统计</td>
  </tr>
  <tr>

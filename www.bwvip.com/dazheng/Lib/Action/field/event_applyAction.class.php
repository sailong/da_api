...
class event_applyAction extends field_publicAction
{
	...
	public function event_apply_add()
	{
		...
		$fenzhan=D('fenzhan_tbl')->fenzhan_list_pro(" and field_uidfield_uid='".$_SESSION['field_uid']."'  ");
	...
	public function event_apply_edit_action()
	{
		if(M()->autoCheckToken($_POST))
		{
			...
			$this->success("修改成功",U('field/event_apply/event_apply',array('event_id'=>$data['event_id'],'fenzhan_id'=>$data["fenzhan_id"],'event_apply_state'=>1)));
		
		}
		else
		{
			$this->error("参数错误或来路非法",U('field/event_apply/event_apply',array('event_id'=>$data['event_id'],'fenzhan_id'=>$data["fenzhan_id"],'event_apply_state'=>1)));
		}

...
/**
 *    #Case		bwvip
 *    #Page		App_versionAction.class.php (客户端版本  上传审核中)
 *
 *    @author		Zhang Long
 *    @E-mail		123695069@qq.com
 */
class app_version_upAction extends AdminAuthAction
{
	...
	{
		$list=D("app_version_up")->app_version_list_pro();
	...
	public function app_version_add_action()
	{
		if(M()->autoCheckToken($_POST))
		{
			...
			$list=M("app_version_up")->add($data);
			if($list!=false)
			{
	...
	public function app_version_edit()
	{
		...
		{
			$data=M("app_version_up")->where("app_version_id=".intval(get("app_version_id")))->find();
	...
	public function app_version_edit_action()
	{
		if(M()->autoCheckToken($_POST))
		{
			...
			$list=M("app_version_up")->save($data);
			if($list!=false)
			{
	...
	public function app_version_delete_action()
	{
		if(post("ids"))
		{
			...
			{
				$res=M("app_version_up")->where("app_version_id=".$ids_arr[$i])->delete();
			}
			echo "succeed^删除成功";
		}
	}
	...
	public function app_version_check_action()
	{
		if(post("ids"))
		{
			...
			for($i=0; $i<count($ids_arr); $i++)
			{
				$res=M()->execute("update tbl_app_version_up set app_version_state=1 where app_version_id=".$ids_arr[$i]." ");
			}
			if($res)
			{
				echo "succeed^审核成功";

...
class fenzhanAction extends AdminAuthAction
{
	...
	public function fenzhan_add_action()
	{
		if(M()->autoCheckToken($_POST))
		{
			...
			$this->success("添加成功",U('admin/fenzhan/fenzhan',array('event_id'=>$data['event_id'])));
		}
		else
		{
	...
	public function fenzhan_edit_action()
	{
		if(M()->autoCheckToken($_POST))
		{
			...
			$this->success("修改成功",U('admin/fenzhan/fenzhan',array('event_id'=>post('event_id'))));
			
		}
		else
		{
			$this->error("不能重复提交",U('admin/fenzhan/fenzhan',array('event_id'=>post('event_id'))));
		}

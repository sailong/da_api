 ...
 *    @Date			2013-08-06
 */
class user_itemAction extends AdminAuthAction
{
	...
	public function user_item()
	{
		...
		if($ticket_id){
			...
			$sel_item_list = M('item')->where("item_id='{$ticket_id}' or parent_id='{$ticket_id}'")->select();
			foreach($sel_item_list as $key=>$val){
				$sel_item_id[$val['item_id']] = $val['item_id'];
			}
			unset($sel_item_list);
			$item_id_sql .= " and ticket_id in('".implode("','",(array)$sel_item_id)."')";
		}
		...
		$list=D("user_ticket")->user_ticket_list_pro("{$item_id_sql}{$price_sql}{$user_ticket_status_sql}");
		
		//$ticket_lists = M('ticket')->where("event_id='{$event_id}'")->select();
		$item_lists = M('item')->select();
		
		$new_item_list = array();
		$sub_item_list = array();
		foreach($item_lists as $key=>$val){
			if($val['parent_id'] == 0){
				$new_item_list[$val['item_id']] = $val;
			}else{
				$sub_item_list[$val['parent_id']][$val['item_id']] = $val;
			}
		}
		
		foreach($new_item_list as $key=>$val){
			$new_item_list[$val['item_id']]['sub'] = $sub_item_list[$val['item_id']];
		}
		...
		$item_list = M('item')->where("item_id in('".implode("','",(array)$ticket_ids)."')")->select();
		
		foreach($item_list as $key=>$val)
		{
			unset($item_list[$key]);
			$item_list[$val['item_id']] = $val['item_name'];
		}
		/* echo '<pre>';
		var_dump($item_list); */
		$this->assign("item_lists",$new_item_list);
	...
	public function user_item_edit()
	{
	...
	public function user_item_edit_action()
	{
	...
	public function user_item_delete_action()
	{
		if(post("ids"))
		{
	...
	public function user_item_check_action()
	{
		if(post("ids"))
		{
			$ids_arr=explode(",",post("ids"));
	...
	public function user_item_detail()
	{
		if(intval(get("user_ticket_id"))>0)
		{
			$data=M("user_ticket")->where("user_ticket_id=".intval(get("user_ticket_id")))->find();

...
class tongjiAction extends AdminAuthAction
{
  ...
  <tr>
    ...
    <td bgcolor="#FFFFFF">'.count($login_all_num).'</td>
		...
		$today_s=strtotime($day." 00:00:00");
		$today_e=strtotime($day." 23:59:59");
		$today_num=M()->query("select count(uid) as num from pre_common_member_profile where reg_source=1203 and regdate>'".$today_s."' and regdate<'".$today_e."' limit 1 ");
		//echo "<li>".$day."(今天)：".$today_num[0]['num']."</li>";
		...
		$today_s=strtotime($day2." 00:00:00");
		$today_e=strtotime($day2." 23:59:59");
		$today_num2=M()->query("select count(uid) as num from pre_common_member_profile where reg_source=1203 and regdate>'".$today_s."' and regdate<'".$today_e."' limit 1 ");
		//echo "<li>".$day2."(昨天)：".$today_num2[0]['num']."</li>";
		
		$day3=date("Y-m-d",(time()-84000*2));
		$today_s=strtotime($day3." 00:00:00");
		$today_e=strtotime($day3." 23:59:59");
		$today_num3=M()->query("select count(uid) as num from pre_common_member_profile where reg_source=1203 and regdate>'".$today_s."' and regdate<'".$today_e."' limit 1 ");
		//echo "<li>".$day3."(前天)：".$today_num3[0]['num']."</li>";
		
		
		$today_s=strtotime(date("Y-m-d",time())." 00:00:00");
		$today_e=strtotime(date("Y-m-d",time()-84000*6)." 23:59:59");
		$today_num4=M()->query("select count(uid) as num from pre_common_member_profile where reg_source=1203 and regdate>'".$today_e."' and regdate<'".$today_s."' limit 1 ");
		//echo "<li>".date("Y-m-d",$today_s)."　～　".date("Y-m-d",$today_e)."(本周)：".$today_num4[0]['num']."</li>";
		
		
		$reg_all_num=M()->query("select count(uid) as num from pre_common_member_profile where reg_source=1203 limit 1 ");
		//echo "<li>总注册人数：".$reg_all_num[0]['num']."</li>";
		...
		$today_s=strtotime(date("Y-m-d",time()-86400)." 00:00:00");
		$today_e=strtotime(date("Y-m-d",time()-86400)." 23:59:59");
		$login_yestoday_num=M()->query("select uid from tbl_app_log where field_uid='1203' and app_log_mod='login' and app_log_addtime<'".$today_e."' and app_log_addtime>'".$today_s."' and (user_agent='iPhone' or user_agent='Android') and uid>0 group by uid ");
		$login_yestoday_num_iphone=M()->query("select uid from tbl_app_log where field_uid='1203' and app_log_mod='login' and app_log_addtime<'".$today_e."' and app_log_addtime>'".$today_s."' and user_agent='iPhone' and uid>0 group by uid ");
		$login_yestoday_num_android=M()->query("select uid from tbl_app_log where field_uid='1203' and app_log_mod='login' and app_log_addtime<'".$today_e."' and app_log_addtime>'".$today_s."' and user_agent='Android' and uid>0 group by uid ");
		
		//今天
		$today_s=strtotime(date("Y-m-d",time())." 00:00:00");
		$today_e=strtotime(date("Y-m-d",time())." 23:59:59");
		$login_today_num=M()->query("select uid from tbl_app_log where field_uid='1203' and app_log_mod='login' and app_log_addtime<'".$today_e."' and app_log_addtime>'".$today_s."' and (user_agent='iPhone' or user_agent='Android') and uid>0 group by uid ");
		$login_today_num_iphone=M()->query("select uid from tbl_app_log where field_uid='1203' and app_log_mod='login' and app_log_addtime<'".$today_e."' and app_log_addtime>'".$today_s."' and user_agent='iPhone' and uid>0 group by uid ");
		$login_today_num_android=M()->query("select uid from tbl_app_log where field_uid='1203' and app_log_mod='login' and app_log_addtime<'".$today_e."' and app_log_addtime>'".$today_s."' and user_agent='Android' and uid>0 group by uid ");
		
		$login_all_num=M()->query("select uid from tbl_app_log where field_uid='1203' and app_log_mod='login' and uid>0 group by uid ");
		...
		echo '<table width="800" border="0" bgcolor="#cccccc">
  <tr>
    <td colspan="3" style="line-height:50px; text-align:center; font-size:18px; font-weight:bold;" bgcolor="#FFFFFF">天津滨海森林登录注册统计</td>
  </tr>
  <tr>
    <td width="25%" rowspan="6" bgcolor="#FFFFFF" align="center">注册</td>
    <td width="35%" bgcolor="#FFFFFF">&nbsp;</td>

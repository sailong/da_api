...
/**
 *    #Case		bwvip
 *    #Page		app_version_upModel.class.php (客户端版本 上传中)
 *
 *    @author		Jack
 *    @E-mail		123695069@qq.com
 */
class app_version_upModel extends Model{
	...
	function app_version_list_pro($bigwhere="", $page_size=20, $sort=" app_version_addtime desc ") 
	{
		...
		$data["item"]=M("app_version_up")->where($where.$bigwhere)->order($sort)->page($page.",".$page_size)->select();
		...
		$data["total"] = M("app_version_up")->where($where.$bigwhere)->count();
	...
	function app_version_select_pro($bigwhere="",$limit=999999, $sort=" app_version_addtime desc ") 
	{
		...
		$data["item"]=M("app_version_up")->where($where.$bigwhere)->order($sort)->limit($limit)->select();
		$data["total"]=M("app_version_up")->where($where.$bigwhere)->count();

...
if($operation == 'save') {
	...
	if($count) {
		$query = DB::query('select sco.*, fil.fieldname from '.DB::table('common_score')." as sco left join ".DB::table('common_field_test_1')." as fil on sco.fieldid=fil.id where uid='$_G[uid]' order by sco.id desc limit $start, $pagesize");
	...
	$id = $_GET['id'];
	$query = DB::query('select sco.*, fil.fieldname from '.DB::table('common_score')." as sco left join ".DB::table('common_field_test_1')." as fil on sco.fieldid=fil.id where sco.id=$id limit 1");

...
$getstat = array();
	...
	//$getstat['usrnickname'];
 $navtitle=$getstat['usrnickname'].'的视频管理';
...
//	$videoid = $_POST["videoid"];
	...
	$row = DB::query("insert into ".DB::table('home_video')."(videoid,vpid,uid,username,dateline,title,content) values($videoid,$vpid,$uid,'$username',$dateline,'$title','$content');");
	if($row) {
		//积分 
...
if($op == 'del'){ 
	$id = $_GET['id'];
	if($id) {
		DB::query("DELETE FROM ".DB::table('home_video')." WHERE vid=$id");
		DB::query("DELETE FROM ".DB::table('home_videopath')." WHERE vpid=$id");		
	//	DB::query("DELETE FROM ".DB::table('home_comment')." WHERE id  =$id AND idtype='video'");
      DB::query("DELETE FROM ".DB::table('home_feed')." WHERE id IN (".simplode($newids).") AND idtype='video'");
		...
		showmessage('do_success', 'home.php?mod=spacecp&ac=vupload&op=list');
	}
	/*if(deletevideos($id)){
		showmessage('do_success', 'home.php?mod=spacecp&ac=vupload&op=list');
	}*/
...
if($op == 'edit'){ 
	...
	$query = DB::query("SELECT v.vid,v.title,v.uid,v.dateline,v.recomm,v.content,vp.* FROM ".DB::table('home_video')." v 
										inner join ".DB::table('home_videopath')." vp ON vp.vpid = v.vpid  WHERE  v.vpid='$id'"); 
...
if($op == 'editseq'){ 
	...
	$query = DB::query("update ".DB::table('home_video')." set title='$title',content='$content'  WHERE  vpid='$id'"); 
		showmessage('do_success', 'home.php?mod=spacecp&ac=vupload&op=list');
}
if($op == 'list'){ 
	$pagesize = 21;
...
ckstart($start, $pagesize);
	...
	$count = DB::result_first("select count(*) from ".DB::table('home_video')." hv inner join ".DB::table('home_videopath')." hvp on hvp.vpid=hv.vpid where hv.uid='$uid' order by hv.vid desc");
 
	$videolist = array();
	if($count) {
		$sql="select hv.vid, hv.title, hv.uid, hv.dateline, hv.recomm, hv.content, hvp.* from ".DB::table('home_video')." hv inner join ".DB::table('home_videopath')." hvp on hvp.vpid=hv.vpid where hv.uid='$uid' order by hv.vid desc limit $start, $pagesize";
		//echo $sql;
		$query = DB::query($sql);
		while($row = DB::fetch($query)) {
			$row['title'] = (strlen($row['title']) > 9) ? mb_substr($row['title'], '0', '8', 'utf-8').'...' : $row['title'];
			$videolist[] = $row;
		}
	}

 	$multi = multi($count, $pagesize, $page, "home.php?mod=spacecp&ac=vupload&op=list");
} 
//视频分类
$uid = $space['uid'];
$listvideotype = array();
...
$templates = 'home/spacecp_vupload';

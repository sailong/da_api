...
if($op == 'list'){ 
	...
	$videolist = array();
	if($count) {
		$query = DB::query("select hv.vid, hv.title, hv.uid, hv.dateline, hv.recomm, hv.content, hvp.* from ".DB::table('home_video')." hv inner join ".DB::table('home_videopath')." hvp on hvp.vpid=hv.vpid where hv.uid='$uid' order by hv.vid desc limit $start, $pagesize");

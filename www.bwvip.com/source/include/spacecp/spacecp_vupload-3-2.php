...
$operation = in_array($_GET['aa'], array('list', 'vup')) ? trim($_GET['aa']) : 'list';

if(in_array($operation, array('list', 'vup'))) {
...
if($op=='fenlei'){
	...
	$sql = "select * from ".DB::table('home_videotype')." where uid = $uid and vtypename = '$vtypename'";
	$rs  =  DB::fetch($sql);
	if($rs){

...
?>
	...
	if($_GET['do'] == 'del') {
		...
		if(empty($_GET['c'])) {
			...
			$dir = $dir.'/'.$num.'.jpg';
			$array = array('image/gif', 'image/jpeg', 'image/jpg', 'image/pjpeg', 'image/png');
			if(in_array($file['imglogo']['type'], $array)) {
				$move = move_uploaded_file($tmp_name, $dir);
				if($move) {
					$arr['imglogo'] = $dir;
					$row = DB::insert('home_advert', $arr);
					if($row) {
						cpmsg('添加成功', 'action=misc&operation=advert&do=addadvert');
					} else {
						unlink($dir);
						cpmsg('添加失败', 'action=misc&operation=advert&do=addadvert');
					}
				} else {
					cpmsg('上传文件失败', 'action=misc&operation=advert&do=addadvert');
				}
			} else {
				cpmsg('文件类型不符合', 'action=misc&operation=advert&do=addadvert');
			}
		} else {
			unset($arr['advertid']);
		...
		echo "<form action='admin.php?action=misc&operation=advert&do=save&c=1' method='post' enctype='multipart/form-data'><table width='96%; style='border:1px solid gray'><input type='hidden' name='advertid' value='".$advert['id']."' /><tr style='height:30px'><td style='width:10%'>广告名称</td><td><input type='text' name='advertname' value='".$advert['advertname']."' style='width:300px' maxlength='20' /></td></tr><tr><td>链接地址</td><td><input type='text' name='adverturl' value='".$advert['adverturl']."' style='width:300px' maxlength='100' /></td></tr><tr><td>广告分类</td><td><select name='adverttype' style='widht:160px'>".$option."</select></td></tr><tr><td>广告描述</td><td><input type='text' name='content' value='".$advert['content']."' style='width:300px; height:80px' maxlength='300' /></td></tr><tr><td>广告地址</td><td>".$advert['imglogo']."</td></tr><tr><td colspan='2' style='text-align:left; padding-left:200px; padding-top:20px'><input type='submit' name='submit' value='提交' style='border:1px solid gray' /></td></tr></table></form>";
	} else {
		...
		$count = DB::result(DB::query("select count(*) from ".DB::table('home_advert')." as ha left join ".DB::table('home_adverttype')." as hat on hat.tid=ha.adverttype"));
		if($count) {
			$query = DB::query("select ha.*, hat.tname from ".DB::table('home_advert')." as ha left join ".DB::table('home_adverttype')." as hat on hat.tid=ha.adverttype order by id desc limit $start, $pagesize");
			while($row = DB::fetch($query)) {
				$str .= "<tr id=".$row['id']." onmouseover='showcolor(".$row['id'].")' onmouseout='clearcolor(".$row['id'].")' style='height:30px'><td>".$row['id']."</td><td>".$row['advertname']."</td><td>".$row['adverturl']."</td><td>".$row['tname']."</td><td>".$row['content']."</td><td><img src=".$row['imglogo']." width='150' height='80' /></td><td><a href='admin.php?action=misc&operation=advert&do=edit&id=".$row['id']."'>修改</a> | <a href='admin.php?action=misc&operation=advert&do=del&id=".$row['id']."'>删除</a></td></tr>";
			}
			$multipage = multi($count, $pagesize, $page, '?action=misc&operation=advert');
			echo $str."</table><a href='admin.php?action=misc&operation=advert&do=addadvert'>添加广告</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href='admin.php?action=misc&operation=advert&do=addtype'>添加分类</a><br />".$multipage;
		}
	}
}
?>

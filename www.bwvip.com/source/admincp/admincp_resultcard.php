...
if($operation == 'manage') {
	   ...
	   $query = DB::query("SELECT cs.id, cs.uid,cs.dateline,cs.status,cs.total_score,cmp.realname,cmp.mobile FROM ". DB::table('common_score')." as cs LEFT JOIN ".DB::table('common_member_profile')." as cmp ON cmp.uid=cs.uid  " .$where. "   ORDER BY cs.id desc limit ".$start_limit."," .$_G['setting']['guess_perpage']." " ); 
	   	$query_num = DB::query("SELECT cs.id FROM ". DB::table('common_score')." as cs LEFT JOIN ".DB::table('common_member_profile')." as cmp ON cmp.uid=cs.uid  " .$where. " " );
   ...
   /*add 下面的 ac =='sure' insert 添加写入数据库*/
	if($_G['gp_ac']=='sure')
	{
		...
		foreach($mobiles as $mob){
            ...
            $guid=getgpc($mob);
				/*查询是否已经存在*/
				$result = DB::fetch_first( " SELECT * FROM ".DB::table('common_score')." where uid='".$guid."' and fuid='".getgpc('fuid')."' and sais_id='".getgpc('sais_id')."' and tee = '".getgpc('dongk')."' and dateline = '".$dateline."' ");

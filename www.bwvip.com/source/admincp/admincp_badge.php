...
$operation = $operation ? $operation : 'list';
	...
	if($operation == 'list') {
	$fenz = "徽章类型：<select id=\"badge_id\" name=\"badge_id\">   
											<option value=\"\" style=\"background-color:#efefef\">用户身份类徽章</option> 
											<option value=\"2\">中高协教练徽章</option> 
											<option value=\"1\">俱乐部教练徽章</option> 
											<option value=\"4\">外籍教练徽章</option> 
											<option value=\"3\">港澳台教练徽章</option> 
											<option value=\"18\">中高协裁判徽章</option> 
											<option value=\"20\">普通从业者徽章</option> 
											<option value=\"7\">球场总经理徽章</option> 
											<option value=\"19\">球童徽章</option> 
											<option value=\"8\">草坪专家徽章</option> 
											<option value=\"9\">专家徽章</option> 
											<option value=\"\" style=\"background-color:#efefef\">企业用户类徽章</option>
											<option value=\"5\">皇冠杯城市挑战赛徽章</option>  					
		...
		showformheader('badge&operation=list');
		showtableheader('按 UID：<input class="text" name="suid" size="10" onkeyup="value=value.replace(/[^\d]/g,\'\')" onbeforepaste="clipboardData.setData(\'text\',clipboardData.getData(\'text\').replace(/[^\d]/g,\'\'))" >
		...
		是否审核：<input name="getting_badge_or_not" value="1" type="radio" >已审核 <input name="getting_badge_or_not" value="0" type="radio" >未审核
		...
		$pstr='';
		if($_G['gp_suid']>0){
	      $strwhere=$strwhere.' and uid='.$_G['gp_suid'];  
	      $strwhere1=$strwhere1.' and a.uid='.$_G['gp_suid']; 
		 }
		if($_G['gp_badge_id']>0){
	      $strwhere=$strwhere.' and badge_id='.$_G['gp_badge_id'];  
	      $strwhere1=$strwhere1.' and a.badge_id='.$_G['gp_badge_id']; 
		  $pstr=$pstr.'&badge_id='.$_G['gp_badge_id'];
		 }
		
		if(isset($_G['gp_getting_badge_or_not'])){
	      $strwhere=$strwhere.' and getting_badge_or_not='.$_G['gp_getting_badge_or_not'];  
	      $strwhere1=$strwhere1.' and a.getting_badge_or_not='.$_G['gp_getting_badge_or_not'];
		  $pstr=$pstr.'&getting_badge_or_not='.$_G['gp_getting_badge_or_not']; 
		 }
		 /*时间的条件 search*/
		if(isset($_G['gp_start_time']) && isset($_G['gp_end_time'])){
			if($_G['gp_start_time']!='' && $_G['gp_end_time']!=''){ 
				$strwhere.=$strwhere ." and addtime >= '".getgpc('start_time')."' and addtime <= '".getgpc('end_time')."' ";
				$strwhere1.=$strwhere1 ." and a.addtime >= '".getgpc('start_time')."' and a.addtime <= '".getgpc('end_time')."' ";				
		        $pstr=$pstr.'&start_time='.$_G['gp_start_time'].'&end_time='.$_G['gp_end_time']; 
			}
		}
		...
		$record_amount = $uabg->get_record_amount_by_automatic_verifying(0,$strwhere);

		$multipage = multi($record_amount, $len_per_page, $page, ADMINSCRIPT."?action=badge$pstr&operation=list&submit=yes".$urladd.'&want_search_num='.$len_per_page);


		$uabg = new user_applying_badges_getter();
		$rows = $uabg->get_record_by_automatic_verifying_and_start_and_len_per_page(0,$start,$len_per_page,$strwhere1);
...
if($rows[$i]["realname"])
{
	$rrrealname=$rows[$i]["realname"];
}else{
	$rrrealname=$uid;
}
			if($rows[$i]["getting_badge_or_not"] == 0){
				echo showtablerow('', array('class="td25"', 'class="td28"'),array('<a href=space-uid-'.$uid.'.html target=_blank>'.$rrrealname.'</a>',$badge,$el,$handle_passing_link.'&nbsp;'.$handle_deleting_link));

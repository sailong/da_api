...
$todo_game_over  = fenz_select($fenz_arr,'todo_game_over','disable(\'no\')');
...
$gp_where=" addtime >= '".strtotime("2012-01-01 13:48:12")."'  and  addtime <= '".strtotime("2013-01-01 13:48:12")."'  ";
}  
	/*删除动作*/
	if(isset($_G['gp_dazbm_delete'])){
		...
		$query = DB::query("SELECT `realname`,`cahdian`,`apply_status`,`pay_way`,`uid`,`bm_id`,`pay_status`,`apply_status`,`moblie`,`addtime`,`telephone` FROM ". DB::table('home_dazbm_copy')." " .$where. " ORDER BY bm_id desc limit ".$start_limit."," .$_G['setting']['dazbm_perpage']." " );
		...
		if(isset($_G['gp_export'])){
              ...
              */
			  ...
			  $query = DB::query("SELECT * FROM ". DB::table('home_dazbm_copy')." " .$where. " ORDER BY bm_id desc " );
			  while($result_list = DB::fetch($query)){
				  $export_list[] = $result_list;
                  /*批量修改汽车*/
                  //DB::update("home_dazbm_copy",array('car_brand'=>$rows[mt_rand(0,97)]),array("uid"=>$result_list['uid']));
			  }
		...
		$query_num = DB::query("SELECT `bm_id` FROM ". DB::table('home_dazbm_copy')." " .$where. " " );
...
{
   $bm_id =  $_G['gp_bm_id'];
   DB::query("DELETE FROM ".DB::table('home_dazbm_copy')." WHERE bm_id IN (".$bm_id.")");
...
elseif($operation == 'game_over') /*用户打完比赛*/
{
   ...
   DB::query("UPDATE  ".DB::table('home_dazbm_copy')." SET `gameover`=1  WHERE bm_id IN (".$bm_id.")");
   DB::query("UPDATE  ".DB::table('home_dazbm_copy')." SET `city_fenz`='".$fenz_name."' WHERE bm_id IN (".$bm_id.")");
...
elseif($operation == 'edit') /*用户报名资料编辑*/
{
	...
	if($html) {
     ...
     $bm_info = DB::fetch_first("SELECT * FROM ".DB::table('home_dazbm_copy')." WHERE bm_id='".$_G['gp_bm_id']."'");
...
elseif($operation == 'approve'){
   ...
   DB::update("home_dazbm_copy",$where, array('bm_id'=>$_G['gp_bm_id']));
...
elseif($operation == 'remark_add'){
   ...
   DB::update("home_dazbm_copy",array('remark_type'=>$data['rm_type_id']),array('bm_id'=>$data['bm_id']));
...
elseif($operation=="pay"){
	...
	DB::update("home_dazbm_copy",array('pay_status'=>$updata_status),array("bm_id"=>$bm_id));

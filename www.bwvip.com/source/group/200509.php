...
$query = DB::query("SELECT uid, fans_count, follow_count,topic_count,username FROM jishigou_members  where uid=$uid");
		...
		$fans_count=empty($value['fans_count'])?'0':$value['fans_count'];
		$follow_count=empty($value['follow_count'])?'0':$value['follow_count'];
		$topic_count=empty($value['topic_count'])?'0':$value['topic_count'];	
...
$wbtag='weibo/index.php?mod=user_tag&uid='.$uid;//微博标签

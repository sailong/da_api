...
//改为真实名字
$realname = DB::result_first("SELECT realname FROM ".DB::table('common_member_profile')."where uid=".$$feedarr['uid']);
 $feedarr['username'] = $realname;
if(!$realname) {
$realname=$feedarr['username'];
}
...
function feed_publish($id, $idtype, $add=0) {
	...
	switch ($idtype) {
		...
		case 'picid':
			...
			break;
			case 'vpid':		 
			$query = DB::query("SELECT v.vpid,v.uid,v.username,v.dateline,v.hot,v.title,p.images  from ".DB::table('home_video')." as v left join ".DB::table('home_videopath')." as p on v.vpid=p.vpid where v.vpid='$id'");
			if($value = DB::fetch($query)) { 
					$setarr['icon'] = 'video';
					$setarr['id'] = $value['vpid'];
					$setarr['idtype'] = $idtype;
					$setarr['uid'] = $value['uid'];
					$setarr['username'] = $value['username'];
					$setarr['dateline'] = $value['dateline'];
					...
					$setarr['hot'] = $value['hot'];
					$status = $value['status'];
					$url = "home.php?mod=space&uid=$value[uid]&do=video&id=$value[vpid]";
					$setarr['image_1'] = $value['images'];
					$setarr['image_1_link'] = $url;
					$setarr['title_template'] = $value['title'];
					$setarr['body_template'] = $value['content'];
					$setarr['body_data'] = array('title' => $value['title']); 
			}
			break;
	}
...
//徐玉枭改为真实名字
$realname = DB::result_first("SELECT realname FROM ".DB::table('common_member_profile')." where uid=".$setarr['uid']);
 $feedarr['username'] = $realname;
if(!$realname) {
$realname=$feedarr['username'];
}
$setarr['username']=$realname;

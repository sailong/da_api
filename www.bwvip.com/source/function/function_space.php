...
if($space['self']) {
				$strwhere = " or uid in (select buddyid from jishigou_buddys where uid='$uid')";
			}
				$query = DB::query("SELECT * FROM ".DB::table('home_feed')." WHERE uid='$uid' $strwhere ORDER BY dateline DESC LIMIT 0,$shownum");
				while ($value = DB::fetch($query)) {
					if(ckfriend($value['uid'], $value['friend'], $value['target_ids'])) {

...
function blog_post($POST, $olds=array()) {
    ...
    /*内部连接优化*/
    $query = DB::query("select * from ".DB::table('keyword_link')." where is_start=1 order by sort_order asc");
    while ($result=DB::fetch($query)) {
    	$keyword_letboo[$result['id']] = "/".$result['keyword']."/";
    	$link_letboo[$result['id']]    = "<a href=\"".$result['link']."\" target=\"_blank\">".$result['keyword']."</a>";
    }
	$message = preg_replace($keyword_letboo, $link_letboo, $POST['message']);
	...
	if($olds['blogid']) {
		...
		//同步更新到tbl_arc  jack_add
		$up=DB::query("update tbl_arc set arc_content='".$message."' where arc_id='".$blogid."' ");

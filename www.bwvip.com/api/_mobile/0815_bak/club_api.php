...
if($ac=="club_index_nologin")
{
	...
	if($max_page>=$page)
	{
			...
			$list=DB::query("select tid,uid,roottid,(select realname from ".DB::table("common_member_profile")." where uid=jishigou_topic.uid) as username,content,content2,replys,forwards,dateline,(select photo from jishigou_topic_image where tid=jishigou_topic.tid limit 1) as photo,voice,voice_timelong from jishigou_topic type<>'reply' order by dateline desc limit $page_start,$page_size ");
	...
	if($uid)
	{
		...
		else
		{
			...
			if($max_page>=$page)
			{
					...
					$list=DB::query("select tid,uid,(select realname from ".DB::table("common_member_profile")." where uid=jishigou_topic.uid) as username,content,content2,replys,forwards,dateline,(select photo from jishigou_topic_image where tid=jishigou_topic.tid limit 1) as photo,voice,voice_timelong from jishigou_topic where type<>'reply' and totid='".$uid."' and type='reply' order by dateline desc limit $page_start,$page_size ");
					while($row = DB::fetch($list) )
					{
						if($row['photo'])
						{

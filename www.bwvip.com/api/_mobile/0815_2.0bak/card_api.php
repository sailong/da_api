	...
	if(!empty($event_info))
	{
			...
			*/
			...
			if($now_fz_id)
			{
				...
				if($fenzhan['lun']==2)
				{
				  $strlun="tlcave+tlcave1 as total_score";
				}
				if($fenzhan['lun']==3)
				{
				  $strlun="tlcave+tlcave1+tlcave2 as total_score";
				}
				if($fenzhan['lun']==4)
				{
				  $strlun="tlcave+tlcave2+tlcave3 as total_score";
				}
				if($fenzhan['lun']==0||$fenzhan['lun']==1)
				{
				  $strlun="tlcave as total_score";
				}
				$lnorder=" total_score+avcave  ";
				if($days>=1)
				{
					...
					$list=DB::query("select uid,realname as username,tlcave as today_score,$strlun,cave_1,cave_2,cave_3,cave_4,cave_5,cave_6,cave_7,cave_8,cave_9,cave_10,cave_11,cave_12,cave_13,cave_14,cave_15,cave_16,cave_17,cave_18,avcave,(cave_10+cave_11+cave_12+cave_13+cave_14+cave_15+cave_16+cave_17+cave_18) as lin,isend from ". DB::table('golf_nd_baofen')." where fenz_id='".$now_fz_id."' order by isend desc,$lnorder,lin,cave_18,cave_17,cave_16 ");
				}
				else
				{
					$list=DB::query("select uid,realname as username,tlcave as today_score,$strlun,cave_1,cave_2,cave_3,cave_4,cave_5,cave_6,cave_7,cave_8,cave_9,cave_10,cave_11,cave_12,cave_13,cave_14,cave_15,cave_16,cave_17,cave_18,avcave,(cave_10+cave_11+cave_12+cave_13+cave_14+cave_15+cave_16+cave_17+cave_18) as lin,isend from ". DB::table('golf_nd_baofen')." where fenz_id='".$now_fz_id."' order by isend desc,$lnorder,lin,cave_18,cave_17,cave_16 ");

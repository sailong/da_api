...
if($ac=='rank')
{
	...
	while($row = DB::fetch($list) )
	{
		...
		if($row['voice'])
		{
	        ...
	        $row = array_default_value($row,array('replys','forwards'));
		$topic_list[]=$row;
	}
	...
	if(!empty($event_info))
	{
			...
			if($now_fz_id)
			{
				$fenzhan_sql=" and fenzhan_id='".$now_fz_id."' ";
			}
			
			
			//最大轮数
			$lun_num = DB::result_first("select max(lun) from tbl_baofen where event_id='".$event_id."' ".$fenzhan_sql." and uid >0 limit 1 ");
			
			$query = DB::query(" SELECT baofen_id,uid,event_id,lun,total_score,total_sum_lun as zong_score,score,par,tianshu FROM (select baofen_id,uid,event_id,lun,total_score,total_sum_lun,total_sum_lun as zong_score,score,par,to_days(FROM_UNIXTIME(dateline))-to_days(now()) as tianshu from tbl_baofen where event_id='".$event_id."' and uid >0 ".$fenzhan_sql." order by lun desc,zong_score asc ,tianshu asc) as t2 group by uid order by lun desc,zong_score asc ,tianshu asc  limit 0,$limit");
			...
			while($row = DB::fetch($query))
			{
				...
				{
					$lun_info="";
					$lun_info = DB::fetch_first("select baofen_id,event_id,uid,total_score,score,par from tbl_baofen where event_id='".$event_id."' and fenzhan_id='".$now_fz_id."' and uid='".$row['uid']."' and lun='".$j."' order by dateline asc limit 1 ");
					if($lun_info['score'])
					{
				...
				if(!$row['zong_score'])
				{
					...
					$res=DB::query("update tbl_baofen set total_sum_lun='".$row['zong_score']."' where uid='".$row ['uid']."' and event_id='".$row ['event_id']."'  ");
				}

...
class ModuleObject extends MasterObject
{
	...
	{	
		...
		if ('invite' == $act) {
			...
			if ($my_invite_count > 0) {
				...
				$page_arr = page($my_invite_count,$per_page_num,$query_link,$_config);		
								...
								$sql = "select * from `".TABLE_PREFIX."invite` where `uid`='{$member['uid']}' order by `id` desc {$page_arr['limit']}";
				$query = $this->DatabaseHandler->Query($sql);
				$uids = array();
				...
				while ($row = $query->GetRow())
				{
					...
					$_list[$row['fuid']] = $row;							$uids[$row['fuid']] = $row['fuid'];	
				}
	
				$sql = "select `buddyid` from `".TABLE_PREFIX."buddys` where `uid`='".MEMBER_ID."' and `buddyid` in ('".implode("','",$uids)."')";
				$query = $this->DatabaseHandler->Query($sql);
				$buddys = array();
				while ($row = $query->GetRow()) 
				{
					$buddys[$row['buddyid']] = $row['buddyid'];
				}
				
								$invite_list = array();
				foreach ($_list as $k=>$v) {
					$v['follow_html'] = follow_html($k,isset($buddys[$k]));
					$invite_list[$k] = $v;
				}
		
			}

						$MEMBER_INVITE_CODE = defined('MEMBER_INVITE_CODE') ? MEMBER_INVITE_CODE : '';
    		...
    		if (!empty($invite_msg)) {
    			$replaces = array(
    				...
    				'inviteurl' => $inviteURL,
    				'invite_num' => $this->Config['invite_enable'],
		...
		elseif ('maybe_friend' == $act) {
			...
			if($my_favorite_tags) {
				$sql = "select distinct(`uid`) as `uid` from `".TABLE_PREFIX."tag_favorite` where `tag` in ('".implode("','",$my_favorite_tags)."') order by `id` desc limit 200";
				$query = $this->DatabaseHandler->Query($sql);
				$uids = array();
				while ($row = $query->GetRow()) 
				{
				...
				if($uids) {
					$sql = "select `uid`,`buddyid` from `".TABLE_PREFIX."buddys` where `uid`='".MEMBER_ID."' and `buddyid` in ('".implode("','",$uids)."')";
					$query = $this->DatabaseHandler->Query($sql);
					$buddys = array();
					$buddys2 = array();
					while ($row = $query->GetRow()) 
					...
					while ($row = $query->GetRow()) 
					{
						$buddy_status = isset($buddys[$row['uid']]);
				...
				if($total_record > 0) {
					...
					if($uids && MEMBER_ID>0) {
						...
						$city = isset($_GET['city']) ? $city : $member['city'];
												...
												$sql ="select * from (select * from `".TABLE_PREFIX."topic` where `uid` in (".implode(",",$uids).") and `type` != 'reply' order by `dateline` desc) a group by `uid` ";
		...
		elseif('usertag' == $act) {
				...
				if($user_tagid) 
				{
												$sql = "select * from `".TABLE_PREFIX."user_tag_fields` where `uid` != '".MEMBER_ID."' and `tag_id` in (".implode(",",$user_tagid).") ";
						$query = $this->DatabaseHandler->Query($sql);
						...
						$where = $where_list = " where `uid` in (".implode(",",$member_uids).")";
				}	
	...
	function InviteByEmail()
	{
		...
		foreach($email_list as $email) {
			...
			if (preg_match("~^[-_.[:alnum:]]+@((([[:alnum:]]|[[:alnum:]][[:alnum:]-]*[[:alnum:]])\.)+([a-z]{2,4})|(([0-9][0-9]?|[0-1][0-9][0-9]|[2][0-4][0-9]|[2][5][0-5])\.){3}([0-9][0-9]?|[0-1][0-9][0-9]|[2][0-4][0-9]|[2][5][0-5]))$~i",$email)) {
				...
				$_sql = "select * from `".TABLE_PREFIX."invite` where `femail`='{$email}'";
				...
				$mail_from_username = $this->Member['username'];
				$mail_from_email = 'no-reply@jishigou.net';
				$mail_from_email = $this->Config['site_admin_email'];
				
				$mail_to = $email;
				$mail_subject = "来自好友{$this->Member['username']}的{$this->Config[site_name]}微博邀请";
				$mail_content = "{$this->Member['username']}邀请你加入{$this->Config[site_name]}微博!

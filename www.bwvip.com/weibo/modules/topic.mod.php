	...
	function Main()
	{
		...
		if($uid != MEMBER_ID || $_GET['mod_original']) {
           ...
           //徐玉枭添加会员
				if($uid){
					$sql = "SELECT b.*,m.*
						FROM pre_guanxi AS b
						LEFT JOIN ".DB::table('members')." AS m
						ON m.`uid` = b.`userid`
						WHERE b.`compid`='{$uid}' and iscomp=1";
				$query = DB::query($sql);

				while ($row = DB::fetch($query)) {
					if(($row['uid']==MEMBER_ID)&&MEMBER_ID>0){
					$ispublish =1;
					};
				}
				}
		...
		//徐玉枭添加球星和赛事
				...
				$groupid = DB::result_first("SELECT groupid FROM pre_common_member WHERE uid='{$uid}' ");
//if((($groupid==20)||($groupid==21)||($groupid==22)||($groupid==23)||($groupid==24)||($groupid==25))&&MEMBER_ID>0){
					if(MEMBER_ID>0){
					 $ispublish =1;
					 //徐玉枭添加@到
						if(($uid != MEMBER_ID)&&($this->Get['fuid'])){
						$gttuid=$this->Get['fuid'];
						$gttnickname = DB::result_first("SELECT nickname FROM ".DB::table('members')." WHERE uid='{$gttuid}' ");
						$gttnickname='@'.$gttnickname.' ';
						}
...
//徐玉枭$options['perpage'] = $per_page_num; 改为5条
		$options['perpage'] = 5;
    ...
    $buddys = array();
			...
			$options['fuid'] = $member['uid'];   //angf do it 2012/5/3
	...
	function Follow()
	{
		...
		$keyword = $this->Post['nickname'] ? $this->Post['nickname'] : $this->Get['nickname'];
				...
				$per_page_num = empty($this->ShowConfig['topic']['follow']) ? 10 : $this->ShowConfig['topic']['follow']+50;
		$gets = array(
			'mod' => $_GET['mod_original'] ? get_safe_code($_GET['mod_original']) : $this->Module,
			'code' => $this->Code,
			'gid' => $this->Get['gid'],
    	...
    	if (!empty($keyword)) {
   	   ...
   	   /*强制关注人名单 angf do it 2012/6/7*/
            $checking =  DB::result_first("select count(*) from ".DB::table('buddys')." where uid='".$member['uid']."'  and buddyid = '1000457'");
            if(!$checking){
                $uids =array('1900089');
                foreach($uids as $id){
                    DB::insert('buddys', array('uid'=>$member['uid'],'buddyid'=>$id));
                }
				DB::query(" UPDATE ".DB::table('members')." SET  `follow_count` = follow_count+24  where uid='".$member['uid']."' ");
            }

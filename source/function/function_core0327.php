...
function getusrarry($uid) {
	...
	$spnickname = DB::result_first("SELECT nickname FROM jishigou_members WHERE uid='$uid'");
	
	if(strlen(DB::result_first("SELECT nickname FROM jishigou_members WHERE uid='$uid'")))
	{$spnickname = DB::result_first("SELECT nickname FROM jishigou_members WHERE uid='$uid'");}
	else
	{$spnickname = $usrusername;}
	
	if(strlen(DB::result_first("SELECT nickname FROM jishigou_members  WHERE uid='$selfuid'")))
	{$usrnickname = DB::result_first("SELECT nickname FROM jishigou_members  WHERE uid='$selfuid'");	}
	else
...
if($str1[1]=='mp4')
  ...
  {
	 $cmd = '/usr/bin/mencoder '.$srcfile.' -o '.$out.'.mp4 -af volume=10 -aspect 16:9 -of avi -noodml -ovc x264 -x264encopts bitrate=500:level_idc=41:bframes=3:frameref=2: nopsnr: nossim: pass=1: threads=auto -oac mp3lame';
  }
  $res = shell_exec($cmd);
 //得到图片
    $cmd = '/usr/local/bin/ffmpeg  -i '.$out.'.flv -y -f image2 -t 0.001 -s 352x240 '.$out.'.jpg';
  $res = shell_exec($cmd);

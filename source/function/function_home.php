	...
	if(!isset($_G[$var])) {
		if($uid == $_G['uid'] && $_G['member']['uid']) {
			...
			$_G[$var]['self'] = 1;
			 ...
			 $realname=getrname($uid);
			$query = DB::query("SELECT uid,cardid,email,password,status,emailstatus,avatarstatus,videophotostatus,adminid,groupid,groupexpiry,extgroupids,regdate,credits,notifysound,timeoffset,newpm,newprompt,accessmasks,allowadmincp,onlyacceptfriendpm,conisbind,iscomp,bound,cardid,'$realname' as username,'$realname' as realname  FROM ".DB::table('common_member')." WHERE uid='$uid'"); //原来dz
            //$query = DB::query("SELECT cm.*,cmp.realname FROM ".DB::table('common_member')." as cm LEFT JOIN ".DB::table('common_member_profile')." as cmp ON cmp.uid = cm.uid  WHERE cm.uid='$uid'");

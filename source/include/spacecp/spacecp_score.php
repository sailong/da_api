...
if($operation == 'save') {
	...
	$scorelist['dateline'] = date('Y-m-d H:i:s', $scorelist['dateline']);
	...
	if($_GET['id']) {
		$scorelist = DB::fetch_first("select cs.*, cf.fieldname, cd.name from ".DB::table('common_score')." as cs left join ".DB::table('common_field')." as cf on cf.uid=cs.fuid left join ".DB::table('common_district')." as cd on cd.id=cs.province where cs.id='".$_GET['id']."' and cs.uid='".$_G['uid']."' and cs.status='0' and cs.ismine='0' order by cs.dateline asc");
		$scorelist['dateline'] = date('Y-m-d H:i:s', $scorelist['dateline']);
		foreach($scorelist as $key=>$val) {
			if(in_array($key, $array)) {
				$scorelist[$key] = array();
		...
		$countscore = DB::result(DB::query("select count(*) from ".DB::table('common_score')." where status='0' and uid='".$_G['uid']."' and ismine='0'"));
		$query = DB::query("select cs.*, cf.fieldname, cd.name from ".DB::table('common_score')." as cs left join ".DB::table('common_field')." as cf on cf.uid=cs.fuid left join ".DB::table('common_district')." as cd on cd.id=cs.province where cs.uid='".$_G['uid']."' and cs.status='0' and cs.ismine='0' order by cs.dateline asc limit $start, $pagesize");
...
function getscore($type, $uid, $start, $pagesize, $array) {
	$query = DB::query("select cs.*, cf.fieldname, cd.name from ".DB::table('common_score')." as cs left join ".DB::table('common_field')." as cf on cf.uid=cs.fuid left join ".DB::table('common_district')." as cd on cd.id=cs.province where cs.uid='".$uid."' and cs.status='".$type."' and cs.ismine='0' order by cs.dateline desc limit $start, $pagesize");
	while($row = DB::fetch($query)) {
...
function getmulti($type, $uid, $pagesize, $page, $operation) {
	$count = DB::result(DB::query('select count(*) from '.DB::table('common_score')." as cs left join ".DB::table('common_field')." cf on cf.uid=cs.fuid left join ".DB::table('common_district')." as cd on cd.id=cs.province where cs.uid='".$uid."' and status='".$type."' and ismine='0'"));
	$theurl = 'home.php?mod=spacecp&ac=score&op='.$operation;

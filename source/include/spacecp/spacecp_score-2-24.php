...
if($operation == 'save') {
	...
	if($arr) {
		...
		if($row) {
			/*
			$host = 'http://121.101.216.67/home.php?mod=spacecp&ac=cut&uid='.$_G['uid'].'&id='.$id;
			...
			$img = imagegrabwindow($handle, '0');
			$browser->Quit();
			*/
			system('import -window root mypicture.jpg');

			$path = $dir.'/score_'.date('YmdHis').'.png';
			imagepng($img, $path);
			...
			ImageDestroy($im);
			unlink($path);
			DB::query("update ".DB::table('common_score')." set thumb='".$thumb."' where uid='".$_G['uid']."' and id='".$id."'");
	...
	$count = DB::result(DB::query('select count(*) from '.DB::table('common_score')." as cs left join ".DB::table('common_field')." as cf on cf.id=cs.fieldid where cs.uid='$_G[uid]'"));
	if($count) {
		$query = DB::query('select cs.*, cf.fieldname from '.DB::table('common_score')." as cs left join ".DB::table('common_field')." as cf on cf.id=cs.fieldid where cs.uid='$_G[uid]' order by cs.id desc limit $start, $pagesize");
	...
	$id = $_GET['id'];
	$arr = DB::fetch_first('select cs.*, cf.fieldname from '.DB::table('common_score')." as cs left join ".DB::table('common_field')." as cf on cf.id=cs.fieldid where cs.id=$id");
...
if($operation == 'par') {
	...
	if($num <= '3') {
		...
		$query = DB::query('select id, fieldname from '.DB::table('common_field')." where $where='".$id."' order by id desc");
...
function ResizeImage($im, $maxwidth, $maxheight, $name) {
	...
	if(($maxwidth && $width > $maxwidth) || ($maxheight && $height > $maxheight)) {
		...
		if(function_exists('imagecopyresampled')) {
			...
			imagecopyresampled($newim, $im, 0, 0, 10, 160, $newwidth, $newheight, $width, $height);
		} else {
			$newim = imagecreate($newwidth, $newheight);
			imagecopyresized($newim, $im, 0, 0, 10, 160, $newwidth, $newheight, $width, $height);
		}

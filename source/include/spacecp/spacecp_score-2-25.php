...
if($operation == 'save') {
	...
	$id = $_GET['id'];
	$arr = DB::fetch_first('select cs.*, cf.fieldname, cd.name from '.DB::table('common_score')." as cs left join ".DB::table('common_field')." as cf on cf.id=cs.fieldid left join ".DB::table('common_district')." as cd on cd.id=cs.province where cs.id=$id");

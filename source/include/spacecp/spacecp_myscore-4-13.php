...
if($operation == 'save') {
	...
	$arr['uploadimg'] = $file;
	if($do == 'scoreadd') {
		$row = DB::insert('common_myscore', $arr);
		$url = 'home.php?mod=spacecp&ac=myscore';
	} elseif($do == 'edit') {
		$row = DB::update('common_myscore', $arr, array('id'=>$arr['id'], 'uid'=>$_G['uid']));
	...
	$count = DB::result(DB::query('select count(*) from '.DB::table('common_myscore')." as cs left join ".DB::table('common_field')." cf on cf.uid=cs.fuid left join ".DB::table('common_district')." as cd on cd.id=cs.province where cs.uid='".$_G['uid']."'"));
	if($count) {
		$query = DB::query('select cs.*, cf.fieldname, cd.name from '.DB::table('common_myscore')." as cs left join ".DB::table('common_field')." as cf on cf.uid=cs.fuid left join ".DB::table('common_district')." as cd on cd.id=cs.province where cs.uid='".$_G['uid']."' order by cs.id desc limit $start, $pagesize");
	...
	$id = $_GET['id'];
	$scorelist = DB::fetch_first('select cs.*, cf.fieldname, cd.name from '.DB::table('common_myscore')." as cs left join ".DB::table('common_field')." as cf on cf.uid=cs.fuid left join ".DB::table('common_district')." as cd on cd.id=cs.province where cs.id=$id and cs.uid='".$_G['uid']."'");
	...
	$id = $_GET['id'];
	$row = DB::query('delete from '.DB::table('common_myscore')." where id='$id'");
	if($row) {

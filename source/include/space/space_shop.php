...
$count = DB::result_first("select count(*) FROM ".$ecname."goods where  is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0 AND goods_id=$goods_id");
 ...
 //购物车
  if($op=='cart'){
	...
	$shoplist = array();
	if($count) {
		$query = DB::query("SELECT *  from ".$ecname."goods where  is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0 $strwhere order by sort_order asc  LIMIT $start,$perpage");
		while ($value = DB::fetch($query)) {
			$shoplist[] = $value;
		}
	}
 ...
 if($ac)
{
	$templates='home/'.$gropid.'_shop'.$ac;

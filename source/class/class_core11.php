...
class discuz_core {
    ...
    /*
    * 微博最近动态
    * at_new fans_new 最新 @提示 消息提示
    * return array*/
    function _weibo_trends(){
        $result = "";
        if($this->var['uid']){
          $result =DB::fetch_first( " select `newpm`,`qun_new`,`comment_new`,`fans_new`,`at_new`,`favoritemy_new`,`vote_new`,`topic_new` from ultrax.jishigou_members where uid =".$this->var['uid']);
        }
        if($result){
          setglobal('weibo_trends', $result);
        }
    }

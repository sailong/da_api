...
class logging_ctl {
	...
	function on_login() {
		...
		if(!submitcheck('loginsubmit', 1, $seccodestatus)) {
			...
			$result = userlogin($_G['gp_username'], $_G['gp_password'], $_G['gp_questionid'], $_G['gp_answer'], 'auto');
			...
			if($result['status'] == -1) {
				if(!$this->setting['fastactivation']) {
					...
					DB::insert('common_member', array(
						...
						'adminid' => 0,
						'groupid' => $this->setting['regverify'] ? 10 : $this->setting['newusergroupid'],
						'regdate' => TIMESTAMP,
						'credits' => $init_arr[0],
						'timeoffset' => 9999
					));
			...
			if($result['status'] > 0) {
				...
				//徐玉枭登陆后跳转
				//$location = 'home.php?mod=space&uid='.$_G['uid'];
...
class register_ctl {
	...
	function on_register() {
		...
		global $_G;
		$_G['gp_regmessage'] = "大正网";
		if($cross_domain == true){
			$_G['gp_username'] = getgpc('registering_username');
			$_G['gp_password'] = getgpc('registering_password');
			$_G['gp_password2'] = getgpc('registering_confirming_password');
			$_G['gp_email'] = $_G['gp_username'];
		}else{
		...
		if($_G['uid']) {
			$ucsynlogin = $this->setting['allowsynlogin'] ? uc_user_synlogin($_G['uid']) : '';
			if($_GET['apply']=='dazbm'){
				$url_forward = "dazbm.php";//dreferer();
			}else{
		...
		if(!submitcheck('regsubmit', 0, $seccodecheck, $secqaacheck) && !$cross_domain) {
	       ...
	       $dreferer = dreferer();
			//徐玉枭更改跳转
			 //$dreferer = 'reglast.php';
			/*报名注册注册时候 对报名的状态行判断-angf作*/
		    if(isset($_G['gp_apply']) && $_G['gp_apply']=='dazbm') $dreferer ='http://bwvip.com/dazbm.php';
			...
			{ 
				...
				if($smcode=='4008109966')
				{
				 ...
				 //showmessage( '已验证', '', array(), array('handle' => true)); 
							}
							else
							{ 
							  
				showmessage( '手机验证码错误！'.$mobile, '', array(), array('handle' => false)); 
								}
				}
			}
			//手机验证完成
                ...
                $sid = trim(intval($_G['gp_sid'])); //angf do it 2012/6/26  注册时识别是那个赛事的用户 主要是对赛事用户进行分组
				$fuid = trim(intval($_G['gp_fuid']));  // 关系ID
			...
			foreach($_G['cache']['fields_register'] as $field) {
				...
				if(!profile_check($field_key, $field_val) && !$cross_domain) {
            ...
            $profile['sid'] = $sid;
			$profile['fuid'] = $fuid;
            $profile['team'] =isset($_GET['team']) ? intval($_G['gp_team']) : 0;
			$field_forum['uid'] = $uid;
			$field_home['uid'] = $uid;
			...
			switch($this->setting['regverify']) {
				...
				case 2:
					...
					$url_forward = $_G['setting']['homestatus'] ? 'home.php?mod=space' : 'home.php?mod=spacecp';
					$url_forward = dreferer();    //这行后加的 为了做大正报名跳转
					break;
				default:
			...
			if($cross_domain == true){
				$href = getgpc('form_address');
			}else{
				$href = str_replace("'", "\'", $url_forward);
			}
			/*报名注册注册时候 对报名用户进行判断-angf作*/
			if(strexists($href,'dazbm')) $locationmessage = 'register_succeed_dazbm';
			/*发送邮件*/
			include libfile('function/dzemail');

...
?>
	...
	var rowtypedata = [
		[
			[1,'', 'td25'],
			[1,'<input type="text" class="txt" name="newuname[]" style="width:80px">', 'td28'],
			[1,'<input type="text" class="txt" name="newname[]" style="width:100px">'],
			[1,'<input type="text" class="txt" name="newurl[]" style="width:180px">'],
			[1,'<input type="text" class="txt" name="newdescription[]" style="width:180px">', 'td26'],
			[1,'<input type="text" class="txt" name="newlogo[]" style="width:180px">']
		]
	]
	</script>
	<?php
		shownav('extended', '空间链接');
		showsubmenu('空间链接');
		showtips('个人空间友情链接管理');
		showformheader('misc&operation=spacelink');
		showtableheader();
		showsubtitle(array('', '用户名', '图片名称', '链接地址', '描述', '图片地址'));
		showsubtitle(array('', '', '', '', '', '', ''));

		$query = DB::query("SELECT link.*, mem.username FROM ".DB::table('common_spacelink')." as link left join ".DB::table('common_member')." as mem on link.uid=mem.uid ORDER BY id desc");
		while($forumlink = DB::fetch($query)) {
			$type = sprintf('%04b', $forumlink['type']);
			showtablerow('', array('class="td25"', 'class="td28"', '', '', 'class="td26"'), array(
				'<input type="checkbox" class="checkbox" name="delete[]" value="'.$forumlink['id'].'" />',
				'<input type="text" class="txt" name="uname['.$forumlink[id].']" value="'.$forumlink['username'].'" style="width:80px" />',
				'<input type="text" class="txt" name="imgname['.$forumlink[id].']" value="'.$forumlink['imgname'].'" style="width:100px" />',
				'<input type="text" class="txt" name="imgurl['.$forumlink[id].']" value="'.$forumlink['imgurl'].'" style="width:180px" />',
				'<input type="text" class="txt" name="description['.$forumlink[id].']" value="'.$forumlink['description'].'" style="width:180px" />',
				'<input type="text" class="txt" name="imglogo['.$forumlink[id].']" value="'.$forumlink['imglogo'].'" style="width:180px" />',
			));
		}
		...
		if($_G['gp_delete']) {
			DB::delete('common_spacelink', "id IN (".dimplode($_G['gp_delete']).")");
			$str = '个人空间友情链删除成功';
		}

		if(is_array($_G['gp_imgname']) && !empty($_G['gp_imgurl']) && !empty($_G['gp_imglogo'])) {
			foreach($_G['gp_imgname'] as $id => $val) {
				$array = DB::fetch_first("select uid from ".DB::table('common_member')." where username='".$_G['gp_uname'][$id]."'");
				DB::update('common_spacelink', array(
					'uid' => $array['uid'],
					'imgname' => $_G['gp_imgname'][$id],
					'imgurl' => $_G['gp_imgurl'][$id],
					'description' => $_G['gp_description'][$id],
					'imglogo' => $_G['gp_imglogo'][$id]
				), array(
					'id' => $id,
				));
				$str = '个人空间友情链修改成功';
		...
		if(is_array($_G['gp_newname']) && !empty($_G['gp_newurl']) && !empty($_G['gp_newlogo'])) {
			foreach($_G['gp_newname'] as $key => $value) {
				if($value) {
					$arr = DB::fetch_first("select uid from ".DB::table('common_member')." where username='".$_G['gp_newuname'][$key]."'");
					DB::insert('common_spacelink', array(
						'uid' => $arr['uid'],
						'imgname' => $value,
						'imgurl' => $_G['gp_newurl'][$key],
						'description' => $_G['gp_newdescription'][$key],
						'imglogo' => $_G['gp_newlogo'][$key],
						'type' => 1,
					));
					$str = '个人空间友情链添加成功';
				}
		...
		updatecache('forumlinks');
		cpmsg($str, 'action=misc&operation=spacelink', 'succeed');
	}
} elseif($operation == 'advert') {  

//添加顶部菜单xyx
$doo=empty($do)?'adlist':$do;

$navmenu[] = array('广告列表', 'misc&operation=advert&do=adlist', $doo == 'adlist');
$navmenu[] = array('添加广告', 'misc&operation=advert&do=addadvert', $doo == 'addadvert');
$navmenu[] = array('分类管理', 'misc&operation=advert&do=tylist', $doo == 'tylist');
$navmenu[] = array('添加分类', 'misc&operation=advert&do=addtype', $doo == 'addtype');
showsubmenu("广告管理",$navmenu);
 
	if($_GET['do'] == 'del') {
		$id = $_GET['id'];
		
		$imglogo = DB::result_first("select imglogo from ".DB::table('home_advert')."  where id='".$id."' ");
		if(file_exists($imglogo))unlink($imglogo);
		
		$row = DB::query("delete from ".DB::table('home_advert')." where id=$id");
			if($row) {
				cpmsg('删除成功', 'action=misc&operation=advert&do=adlist');
			} else {
				cpmsg('删除失败', 'action=misc&operation=advert&do=adlist');
			}	

	} elseif($_GET['do'] == 'addadvert') {
		$query = DB::query("select * from ".DB::table('home_adverttype'));
		$option = "<option value='0'>选择</option>";
		while($row = DB::fetch($query)) {
			$option .= "<option value='".$row['tid']."'>".$row['tname']."</option>";
		}
		echo "<form action='".ADMINSCRIPT."?action=misc&operation=advert&do=save' method='post' enctype='multipart/form-data'><table width='96%; style='border:1px solid gray'><tr style='height:30px'><td style='width:10%'>广告名称</td><td><input type='text' name='advertname' style='width:300px' maxlength='20' /></td></tr><tr><td>链接地址</td><td><input type='text' name='adverturl' style='width:300px' maxlength='100' /></td></tr><tr><td>广告分类</td><td><select name='adverttype' style='widht:160px'>".$option."</select></td></tr><tr><td>广告描述</td><td><input type='text' name='content' style='width:300px; height:80px' maxlength='300' /></td></tr><tr><td>广告地址</td><td><input type='file' name='imglogo' style='width:300px; border:1px solid gray' maxlength='100' /></td></tr><tr><td colspan='2' style='text-align:left; padding-left:200px; padding-top:20px'><input type='submit' name='submit' value='提交' style='border:1px solid gray' /></td></tr></table></form>";
	} elseif($_GET['do'] == 'save') {
		$arr = $_POST;
		$arr['dateline'] = time();
		unset($arr['submit']);
		...
		if(empty($_GET['c'])) {
			$arr['imglogo'] = $dir;
			$row = DB::insert('home_advert', $arr);
			if($row) {
				cpmsg('添加成功', 'action=misc&operation=advert&do=adlist');
			} else {
				unlink($dir);
				cpmsg('添加失败', 'action=misc&operation=advert&do=adlist');
			}
		} else {
			unset($arr['advertid']);
			$arr['imglogo'] = $move ? $dir : ($_POST['imgpath'] ? $_POST['imgpath'] : '');
			unset($arr['imgpath']);
			$row = DB::update('home_advert', $arr, array('id'=>$_POST['advertid']));
			if($row) {
				cpmsg('修改成功', 'action=misc&operation=advert&do=addadvert');
			} else {
				cpmsg('修改失败', 'action=misc&operation=advert&do=addadvert');
			}
		}
	} elseif($_GET['do'] == 'addtype') {
		echo "<form action='".ADMINSCRIPT."?action=misc&operation=advert&do=type' method='post' enctype='multipart/form-data'><table width='96%; style='border:1px solid gray'><tr style='height:30px'><td style='width:10%'>分类名称</td><td><input type='text' name='tname' style='width:300px' maxlength='20' /></td></tr><tr><td colspan='2' style='text-align:left; padding-left:200px; padding-top:20px'><input type='submit' name='submit' value='提交' style='border:1px solid gray' /></td></tr></table></form>";
	} elseif($_GET['do'] == 'type') {
		$arr['tname'] = $_POST['tname'];
		$arr['dateline'] = time();
		$row = DB::insert('home_adverttype', $arr);
		if($row) {
			cpmsg('添加成功', 'action=misc&operation=advert&do=tylist');
		} else {
			cpmsg('添加失败', 'action=misc&operation=advert&do=tylist');
		}
	}elseif($_GET['do'] == 'editype') {
		 $tname = $_POST['tname'];
		 $gid = $_POST['gid'];
		 $turl='action=misc&operation=advert&do=tylist';
		 $flag=DB::update("home_adverttype",array("tname"=>$tname),array("tid"=>$gid));
        if($flag){
            cpmsg("分类修改成功",$turl);
        }else{
            cpmsg("分类修改失败",$turl);
		...
		$id = $_GET['id'];
		$arr = DB::fetch_first("select * from ".DB::table('home_advert')." where id=".$id);
		$row = DB::query("delete from ".DB::table('home_advert')." where id=".$id);
		if($row) {
			unlink($arr['imglogo']);
			cpmsg('删除成功', 'action=misc&operation=advert');
		} else {
			cpmsg('删除失败', 'action=misc&operation=advert');
		}
	} elseif($_GET['do'] == 'edit') {
		$id = $_GET['id'];
		$advert = DB::fetch_first("select * from ".DB::table('home_advert')." where id=$id order by id desc");
		$query = DB::query("select * from ".DB::table('home_adverttype')." order by tid desc");
		$option = "<option value='0'>选择</option>";
		while($row = DB::fetch($query)) {
			if($row['tid'] == $advert['adverttype']) {
		...
		if(file_exists($advert['imglogo'])){
					$strimg="<img src=".$advert['imglogo']." width='300' height='200' />";
				}else{
					$strimg="无图";
				}
		echo "<form action='".ADMINSCRIPT."?action=misc&operation=advert&do=save&c=1' method='post' enctype='multipart/form-data'><table width='96%; style='border:1px solid gray'><input type='hidden' name='advertid' value='".$advert['id']."' /><tr style='height:30px'><td style='width:10%'>广告名称</td><td><input type='text' name='advertname' value='".$advert['advertname']."' style='width:300px' maxlength='20' /></td></tr><tr><td>链接地址</td><td><input type='text' name='adverturl' value='".$advert['adverturl']."' style='width:300px' maxlength='100' /></td></tr><tr><td>广告分类</td><td><select name='adverttype' style='widht:160px'>".$option."</select></td></tr><tr><td>广告描述</td><td><input type='text' name='content' value='".$advert['content']."' style='width:300px; height:80px' maxlength='300' /></td></tr><tr><td>广告地址</td><td><input type='file' name='imglogo' style='width:300px; border:1px solid gray' maxlength='100' /><br />".$strimg."<input type='hidden' name='imgpath' value='".$advert['imglogo']."' /></td></tr><tr><td colspan='2' style='text-align:left; padding-left:200px; padding-top:20px'><input type='submit' name='submit' value='提交' style='border:1px solid gray' /></td></tr></table></form>";
	}elseif($_GET['do'] == 'tydel') {
		$id = $_GET['gid'];
		$row = DB::query("delete from ".DB::table('home_adverttype')." where tid=$id");		
		cpmsg('删除成功', 'action=misc&operation=advert&do=tylist');
		
	}elseif($_GET['do'] == 'tyedit') {
		$gid = empty($_GET['gid']) ? 0 : $_GET['gid'];
		if($gid>0){
  
	$query = DB::query('SELECT tid, tname FROM '.DB::table('home_adverttype').' where tid='.$gid);
		while($value=DB::fetch($query)) {
showformheader("misc&operation=advert&do=editype"); 
	  $strecho="修改推荐分类<br>";
	...
	showformfooter();
	  ...
	  showtableheader("广告分类管理");   //显示表格的第一个tr 的 th
    showsubtitle(array('分类名称','分类ID','操作'));    //显示表格第二个的标题
        ...
        $countnum = DB::result(DB::query("select count(0) num from ".DB::table('home_adverttype')));	   //判断总条数
		
	$query = DB::query('SELECT tid, tname FROM '.DB::table('home_adverttype').' order by tid desc limit '.$start.','.$limitpage);;
			while($v=DB::fetch($query)) {
	 showtablerow('class="td25"','',array($v["tname"],$v["tid"],'<a href="?action=misc&operation=advert&do=tyedit&gid='.$v[tid].'">修改</a>'));   }
	    $theurl = ADMINSCRIPT.'?action=misc&operation=advert&do=tylist'; //地址
      ...
      echo '<p>'.$disppage.'</p>';
		 ...
		 if($tid){
			$strwh=" and ha.adverttype=". $tid; 
		 } 
		 if($ad_search){
			$strwh=" and (ha.advertname like '%".$ad_search."%' or ha.adverturl like '%".$ad_search."%')"; 	 
		 }
		 //增加搜索条件
		 	showformheader('misc&operation=advert&do=adlist');
		 $strecho=" <select name='type' id='sortid' onchange=\"javascript:window.location.href='admin.php?action=misc&operation=advert&do=adlist&tid='+this.options[this.selectedIndex].value\">"; 
	  $strecho.="<option value=''>--选择分类--</option> ";
$query = DB::query('SELECT tid, tname FROM '.DB::table('home_adverttype'));
  ...
  $strecho.="</select>"; 
		...
		$str = "<table width='96%'><tr style='height:30px'><td>广告ID</td><td>广告名称</td><td>链接地址</td><td>广告分类</td><td>广告描述</td><td>图片地址</td><td>操作</td></tr>";
		$count = DB::result(DB::query("select count(*) from ".DB::table('home_advert')." as ha left join ".DB::table('home_adverttype')." as hat on hat.tid=ha.adverttype where  1=1 $strwh")); 
 
		if($count) {
			$sql="select ha.*, hat.tname from ".DB::table('home_advert')." as ha left join ".DB::table('home_adverttype')." as hat on hat.tid=ha.adverttype where  1=1 $strwh order by id desc limit $start, $pagesize";
			$query = DB::query($sql); 
			while($row = DB::fetch($query)) {
				...
				$str .= "<tr id=".$row['id']." onmouseover='showcolor(".$row['id'].")' onmouseout='clearcolor(".$row['id'].")' style='height:30px'><td>".$row['id']."</td><td>".$row['advertname']."</td><td>".$row['adverturl']."</td><td>".$row['tname']."</td><td>".$row['content']."</td><td>".$strimg."</td><td><a href='".ADMINSCRIPT."?action=misc&operation=advert&do=edit&id=".$row['id']."'>修改</a> | <a href='".ADMINSCRIPT."?action=misc&operation=advert&do=del&id=".$row['id']."'>删除</a></td></tr>";
			}
			$multipage = multi($count, $pagesize, $page, '?action=misc&operation=advert');
...
<script type='text/javascript'>
	...
	function del(gid){
	location.href="admin.php?action=misc&operation=advert&do=tydel&gid="+gid;
}
</script>
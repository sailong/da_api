...
cpheader();
$operation = in_array($_GET['operation'], array('member', 'video', 'blog', 'other', 'custom')) ? $_GET['operation'] : 'member';
$id = $_GET['id'];
$do = $_GET['do'];
$c = $_GET['c'];
...
if(($do == 'search' || empty($do)) && $operation != 'other' && $operation != 'custom') {
	if($operation == 'member') {
		...
		if($do == 'search') {
			...
			$thurl = 'admin.php?action=recommend&operation=star&do=search&uid='.$uid.'&username='.$username.'&groupid='.$groupid;
		...
		if($do == 'search') {
			...
			$thurl = 'admin.php?action=recommend&operation=video&do=search&uid='.$uid.'&username='.$username.'&title='.$title;
			$multipage = multi($count, $pagesize, $page, $thurl);
		} else {
		...
		if($do == 'search') {
			...
			while($row = DB::fetch($query)) {
				...
				$list .= "<form action='admin.php?action=recommend&operation=blog&do=recomm&uid=".$row['uid']."&cid=".$row['blogid']."&gid=51' method='post' name='myformval'><tr id=".$row['blogid']." onmouseover='showcolor(".$row['blogid'].")' onmouseout='clearcolor(".$row['blogid'].")'><td><input type='text' name='sort' value='".$sort."' style='width:50px' /></td><td>".$row['blogid']."</td><td>".$row['subject']."</td><td>".$row['username']."</td><td>".date('Y-m-d H:i:s', $row['dateline'])."</td><td>".$rectype."</td><td>".$str."</td></tr></form>";
			}
			$thurl = 'admin.php?action=recommend&operation=blog&do=search&uid='.$uid.'&username='.$username.'&title='.$title;
			$multipage = multi($count, $pagesize, $page, $thurl);
		} else {
	...
	if($_GET['do'] == 'add') {
		...
		$multipage = multi($count, $pagesize, $page, 'admin.php?action=recommend&operation=other');
	...
	if($_GET['do'] == 'addtitle') {
		...
		$option .= "</select>";
		echo "<form action='admin.php?action=recommend&operation=custom&do=save&c=keyword' method='post' enctype='multipart/form-data'>
		...
		while($row = DB::fetch($query)) {
			...
			$str .= "<tr id=".$row['id']." onmouseover='showcolor(".$row['id'].")' onmouseout='clearcolor(".$row['id'].")' style='height:30px'><td>".$row['title']."</td><td>".$row['url']."</td><td>".$row['keyword']."</td><td>".$row['keyurl']."</td><td>".$row['typename']."</td><td><a href='admin.php?action=recommend&operation=custom&do=edit&id=".$row['id']."&c=".$row['flag']."'>修改</a> | <a href='admin.php?action=recommend&operation=custom&do=del&id=".$row['id']."&c=".$row['flag']."'>删除</a>";
		}
		$multipage = multi($count, $pagesize, $page, 'admin.php?action=recommend&operation=custom');
		echo $str."</table>".$multipage."<br /><a href='admin.php?action=recommend&operation=custom&do=addtitle'>添加标题</a>&nbsp;&nbsp&nbsp;&nbsp<a href='admin.php?action=recommend&operation=custom&do=addkeyword'>添加关键词</a>";
	}
}
if($do == 'recomm') {
	$uid = $_GET['uid'];

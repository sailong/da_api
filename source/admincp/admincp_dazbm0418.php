...
elseif($operation == 'remark_list'){
	$query = DB::query("SELECT rm.* , rm_t.remark_type_name FROM ". DB::table('dazapp_remark')." as rm LEFT JOIN ".DB::table('remark_type')." as rm_t ON rm.rm_type_id = rm_t.rm_t_id  where rm.bm_id = '".getgpc('bm_id')."' order by dateline desc" );

...
class register_ctl {
	...
	function on_register() {
		...
		if(!submitcheck('regsubmit', 0, $seccodecheck, $secqaacheck)) {
		    ...
		    if(isset($_G['gp_apply']) && $_G['gp_apply']=='dazbm') $dreferer = dreferer().'dazbm.php';
			include template($this->template);
		...
		/*	//用户注册后直接插入关注表。
			...
			$rand_num=rand(10,20);  
			$query = DB::query("SELECT uid from jishigou_members where topic_count>10 and uid>100000 ORDER BY   lastactivity desc   limit 0,$rand_num");
			while ($value = DB::fetch($query)) {
				$uids1 = $value['uid'];
				$sql = "insert into jishigou_buddys (`uid`,`buddyid`,`dateline`,`buddy_lastuptime`) values ('{$customid}','{$uids1}','{$_G[timestamp]}','{$_G[timestamp]}')";
					DB::query($sql);

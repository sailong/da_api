<?php
	...
	if($operation == 'list') {
		...
		for($i=0;$i<count($rows);$i++){
			...
			$handle_passing_link = '<a href="admin.php?action=badge&operation=handle_passing&uid='.$uid.'&badge_id='.$rows[$i]["badge_id"].'">审核通过</a>';
			$handle_canceling_passing_link = '<a href="admin.php?action=badge&operation=handle_canceling_passing&uid='.$uid.'&badge_id='.$rows[$i]["badge_id"].'">取消审核通过</a>';
...
var_dump($caddie_working_place_info1_1);

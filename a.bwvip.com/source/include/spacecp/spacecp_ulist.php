...
if($op=='list'){ 
	...
	$uids=str_replace("`",",",$uids); 
		 if($uids){DB::query("delete from  ".DB::table('fenzhan_members')."  WHERE uid in ($uids)"); 
		 
		showmessage('已删除成功'.$uids, dreferer(), array(), array('showdialog' => 1, 'closetime' => true));
		}
	...
	if($do=='move') {
	 ...
	 foreach ( $li as $key => $value ) { 
		...
		$count = DB::result_first("SELECT count(1) FROM ".DB::table('fenzhan_members')." WHERE  uid=$value and sid=$uid and fz_id=$fz_id"); 
	        ...
	        if($team){
		    $team_name = DB::result_first("SELECT team_name FROM ".DB::table('golf_team')." WHERE team_id=$team"); 
		   }else
		   {
			   $team=0;
			   $team_name='';
   ...
   $tlist = array();
	...
	$fenz2013 = $_POST['fenz2013']; 
	/*分站的条件 增加2013年search*/
	if($uid==1000333){
		
				$strwhere= " where remark_type=12 or remark_type=23  ";
		/*分站的条件 search*/
		if(isset($_G['gp_fenz']) && $_G['gp_fenz']!='0'){ 
		$strwhere.= " and hot_district like '%".getgpc('fenz')."%' ";
		}
		if($ggid)
		{
		...
		$count = DB::result(DB::query("SELECT count(*) FROM ".DB::table('home_dazbm')."   $strwhere  ORDER BY bm_id DESC"), 0);
		...
		$query = DB::query("SELECT * FROM ".DB::table('home_dazbm')."   $strwhere ORDER BY bm_id DESC LIMIT $start,$perpage");
		while ($value = DB::fetch($query)) {
			$uslist[] = $value;
		}
		}
...
/*修改选手信息*/
	if($op=='edit') {
		...
		$team = $_POST['team'];
		$qdname = $_POST['qdname'];
		$chadian = $_POST['chadian'];
		$realname =trim($_POST['realname']);
		if($id){
		$row = DB::query('update '.DB::table('common_member_profile')." set realname='$realname', chadian='$chadian',team='$team',qdname='$qdname' where uid='$id'");
...
if($op=='fzhy'){  
   ...
   $tlist = array();
	...
	$sql="SELECT * FROM ".DB::table('fenzhan_members')."  WHERE  sid='$uid'  $strwhere ORDER BY team_id,fzm_id DESC LIMIT $start,$perpage";
	$query = DB::query($sql);
		while ($value = DB::fetch($query)) {
			$uslist[] = $value;
		}
...
//添加队籍
   ...
   //修改分站
	...
	{		
		...
		$fz_id=$_POST['fz_id'];
		if($fz_id){DB::query("update ".DB::table('fenzhan')." set fenz_name='$fenz_name',orderby='$orderby',starttime='$starttime',endtime='$endtime'  WHERE fz_id=$fz_id"); 
...
if($op=='add') {
	...
	$realname =trim($_POST['realname']);
	if($ggid){
//$row = DB::query('update '.DB::table('common_member_profile')." set chadian='$chadian', sid='$uid' , is_delete=0 where uid='$ggid' and realname='$realname'");
$row = DB::query('update '.DB::table('common_member_profile')." set chadian='$chadian', sid='$uid' , is_delete=0 where uid='$ggid'");
		showmessage('已添加成功', "home.php?mod=spacecp&ac=ulist", array(), array('showdialog' => 1, 'closetime' => true));
	}
}  
...
if($op=='bfgl'){ 
 ...
 //所有报分人员，18个
    $bf=DB::query("SELECT a.id,a.username,a.password,a.sid,a.fieldid,a.hole,b.realname AS ss_name,c.`realname` AS qc_name FROM `".DB::table('nd_baofen_users')."` AS a LEFT JOIN ".DB::table('common_member_profile')." AS b ON a.sid=b.uid LEFT JOIN pre_common_member_profile AS c ON a.`fieldid`=c.`uid` where a.sid='$uid'  ORDER BY a.id desc");

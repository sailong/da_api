   ...
   showmessage("分组成功",'home.php?mod=spacecp&ac=fenzu');
    ...
    include template('common/footer_ajax');
      ...
      if(submitcheck('member_allotsubmit')){
            ...
            $data['realname']  = getgpc('realname');
            $data['uid']  = getgpc('uid');
			$data['realname'] = DB::result_first("select realname from ".DB::table('home_dazbm')."  where uid='".$data['uid']."' ");
...
if($aa=='addd'){
 ...
 $tt['start_time'] = $start_time; 
            ...
            showmessage("添加成功","home.php?mod=spacecp&ac=fenzu&op=baofen");
}
 $fz_rows = $qc_rows = array();
    $query  =DB::query(" SELECT `fz_id`,`fenz_name` from ".DB::table('fenzhan')." where sid= ".$_G['uid']."");
	while($result = DB::fetch( $query )) {
 ...
 /*搜索动作*/
    if(submitcheck('searchsubmit')) {
        ...
        $fenzhan_members= array();
        /*查看规则是否被删除*/
        $event_rule_query = DB::result_first("SELECT * FROM ".DB::table('event_fenzu_rule_list')." where fenz_id=".$fenz_id." and is_delete=0 ");
        if($event_rule_query){

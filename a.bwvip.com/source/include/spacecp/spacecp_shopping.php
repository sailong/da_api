...
//print_r($ishavearr_time);
if(!$shop_authority_flag){
    header("Location:/");
}
...
$datab=getecprefix();    //"daz_shop.dz_";     //ecshop数据库和对应的前缀
...
if ($operation=='add'){
	...
	if (submitcheck ( 'blogsubmit', 0, $seccodecheck, $secqaacheck )) {
		...
		$realname=getrname($_G["uid"]);     //真实姓名
		//echo $realname;

		/* 如果没有输入商品货号则自动生成一个商品货号 */
		if (empty($_POST['goods_sn']))
		{
			$max_id     = empty($shop["goods_id"]) ? DB::result_first("SELECT MAX(goods_id) + 1 FROM ".$datab."goods") : $shop["goods_id"];
		...
		//不存在就添加
				$flag=DB::query("insert into ".$datab."goods (goods_name,uid,realname,is_on_sale,goods_desc,goods_sn,brand_id,market_price,shop_price,goods_number,warn_number,goods_brief,seller_note,keywords,cat_id,is_best,is_new,is_hot,goods_type,add_time,last_update) values('".$goods_name."',".$_G["uid"].",'".$realname."','".$is_on_sale."','".$goods_desc."','".$goods_sn."',$brand_id,$market_price,$our_price,$goods_number,$warn_number,'".$goods_brief."','".$seller_note."','".$keywords."',$cat_id,$is_best,$is_new,$is_hot,$goods_type,$add_time,$add_time)");

...
$usergroupid = $getstat['groupid']<20?10:intval($getstat['groupid']);
...
if($id) {
	...
	$templates='home/'.$usergroupid.'_privacy';
...
//include template('home/space_privacy');
	...
	//add by xgw on 2012年4月28日 开始
	$blog['messaget']=$blog['message'];   //重新赋值
	$blog['messaget']=strip_tags($blog['messaget']);
	$blog['messaget']=str_replace("&amp;",'',$blog['messaget']);
	$blog['messaget']=str_replace("&nbsp;",'',$blog['messaget']);
	$blog['messaget']=str_replace("\n",'',$blog['messaget']);
	//add by xgw on 2012年4月28日 结束
	$summary = cutstr(strip_tags($blog['messaget']), 80);
...
$templates='home/'.$usergroupid.'_blog_view';
...
include_once(template($templates));
	...
	if(empty($diymode))
	{ $templates='home/blog_list';}
	else
	{$templates='home/'.$usergroupid.'_blog_list';}

}

$navtitle='大正网-提供专业高尔夫新闻，golf教学，球星，视频，赛事，球场预订服务，最纯净高尔夫实名认证互动社区';

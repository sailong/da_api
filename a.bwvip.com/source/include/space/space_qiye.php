...
$groupid=$_GET['groupid'];
	$chaxunqy = array();
	if(!$groupid){
	$query= DB::query("select a.username,a.uid,b.field1,b.field3 from ".DB::table('common_member')." a LEFT JOIN  ".DB::table('common_member_profile')." b ON a.uid=b.uid where a.groupid > 19 order by a.uid");
	}
	else {
	$query= DB::query("select a.username,a.uid,b.field1,b.field3 from ".DB::table('common_member')." a LEFT JOIN  ".DB::table('common_member_profile')." b ON a.uid=b.uid where a.groupid =".$groupid." order by a.uid");
	}
...
if($_GET['qiyeid']){
	...
	showmessage('申请成功等待审核', 'home.php?mod=spacecp&ac=qiye');
}
...
if ($operation=='myqy') {
$myhy=array();
$query = DB::query("SELECT a.userid,a.iscomp,a.compid,a.isuser,b.username FROM ".DB::table("guanxi")." a left join ".DB::table("common_member")." b on a.compid=b.uid WHERE (a.iscomp=1 or a.isuser=1) and a.userid=".$space['uid']);
...
//include template("home/spacecp_qiye");

...
EOF;
		...
		if($membernum > 0) {
			...
			if($uids) {
				...
				$query = DB::query("SELECT m.uid AS uid, m.username AS username,c.realname , m.adminid AS adminid, m.groupid AS groupid, m.credits AS credits, m.conisbind, m.status,
					...
					mc.posts FROM ".DB::table('common_member')." m LEFT JOIN ".DB::table('common_member_count')." mc ON m.uid=mc.uid LEFT JOIN pre_common_member_profile c ON m.uid=c.uid 
					WHERE $conditions");
				...
				while($member = DB::fetch($query)) {
					...
					$members .= showtablerow('', array('class="td25"', '', 'title="'.implode("\n", $memberextcredits).'"'), array(
						...
						($member['conisbind'] ? '<img class="vmiddle" src="static/image/common/connect_qq.gif" /> ' : '')."<a href=\"home.php?mod=space&uid=$member[uid]\" target=\"_blank\">$member[username]</a>","&nbsp;&nbsp;".$member[realname],
						$member['credits'],
						$member['posts'],
		...
		if($membernum) {
			showsubtitle(array('', 'username','realname' , 'credits', 'posts', 'admingroup', 'usergroup', ''));
			echo $members;
	...
	if($uids && is_array($uids)) {
		...
		$query = DB::query("SELECT p.uid,m.username AS username,p.realname,p.gender,p.birthyear,p.birthmonth,p.birthday,p.constellation,
				...
				p.yahoo,p.msn,p.taobao,p.site,p.bio,p.interest,p.field1,p.field2,p.field3,p.field4,p.jigouleibie,p.field6,p.field7,p.field8 FROM ".
	...
	$title = array('realname' => '', 'gender' => '', 'birthyear' => '', 'birthmonth' => '', 'birthday' => '', 'constellation' => '',
		...
		'field1' => '', 'field2' => '', 'field3' => '', 'field4' => '', 'jigouleibie' => '', 'field6' => '', 'field7' => '', 'field8' => '');

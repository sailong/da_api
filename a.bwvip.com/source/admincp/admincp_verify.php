...
EOF;
		...
		if($anchor == 'pass') {
			...
			//微博加v
			DB::query("update jishigou_members set validate='0' where uid='".$uid."'");
				...
				while($value = DB::fetch($query)) {
					if(in_array($_G['gp_verify'][$value['vid']], array('refusal', 'validate'))) {
						...
						if($_G['gp_verify'][$value['vid']] == 'refusal') {
							...
							//微博加v
							DB::query("update jishigou_members set validate='1' where uid='".intval($value['uid'])."'");
							$verify['delete'][] = $value['vid'];
                           //2012.1.15徐玉枭 更改用户组
                            $uid=$value['uid'];
                            $getfield = array();
                            $field = DB::result_first("SELECT field FROM ".DB::table('common_member_verify_info')." WHERE uid='$uid' and verifytype='1' ");	
                            $getfield = unserialize($field);
                            $text = $getfield['jigouleibie'];  
                            $getgroupid = explode('-', $text); 
							$groupidnew = $getgroupid['0'];
							DB::update('common_member', array('groupid'=>$groupidnew), "uid='$uid'");

...
elseif($operation == 'add_card') {
	...
	*/
	...
	$qiuc_select .="</select>";
	$qiuc_select .="选择OUT场：<select name='out_par' id='out_par' onchange=\"load_select(this,'out')\"><option vlaue=''>请选择</option></select>";
	$qiuc_select .="选择IN场：<select name='in_par' id='in_par' onchange=\"load_select(this,'in')\"><option vlaue=''>请选择</option></select>";
...
elseif($operation == 'add') {
   ...
   /*add 下面的 ac =='sure' insert 添加写入数据库*/
	if($_G['gp_ac']=='sure')
	{
		...
		$par_result = DB::fetch_first(" SELECT `par` FROM ".DB::table('common_field')." where uid = '".getgpc('fuid')."'");
		$par ='';
		foreach(explode(',',$par_result['par']) as $key=>$value)
		{

			if($key<=8){ $qin_dong_total +=$value; }
			$par .=$value.'|';
			if($key==8) $par.=$qin_dong_total.'|';
			if($key>8 && key<=17){ $hou_dong_total +=$value; }
			if($key==17) $par.=$hou_dong_total.'|';
			$total +=$value;

...
public function tiaopei_edit()
	{
		...
		{
			$this->error("不能重复提交",U('field/fenzhan/fenzhan_user',array('event_id'=>$data['event_id'],'fenzhan_id'=>$data['fenzhan_id'])));
		}
	...
	public function tiaopei_addaction()
	{
		if(M()->autoCheckToken($_POST))
		{
			...
			$this->success("添加成功",U('field/fenzhan/fenzhan_user',array('event_id'=>$data['event_id'],'fenzhan_id'=>$data['fenzhan_id'])));
		}
		else
		{
			$this->error("不能重复提交",U('field/fenzhan/fenzhan_user',array('event_id'=>$data['event_id'],'fenzhan_id'=>$data['fenzhan_id'])));

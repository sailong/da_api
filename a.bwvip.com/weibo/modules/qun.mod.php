 ...
 *
 * @Date 2011-09-21 14:57:43 1431814756 586123631 34910 $
 *******************************************************************/
...
class ModuleObject extends MasterObject
{
	...
	function view()
	{
		...
		$params['code'] = $this->item;
				...
				$cat_ary = $this->QunLogic->get_category();		$sub_cat = array(
	...
	function manage()
	{
		...
		if ($op == 'icon') {
			...
			if ($count) {
				...
				$query = DB::query("SELECT qu.*,m.nickname  
	...
	function category()
	{
		...
		if ($cat_id) {
			...
			if (!empty($top_cat_ary)) {
				$nav_cat_ary = array_slice($top_cat_ary, 0, 6);
				$first_cat_id = $nav_cat_ary[0]['cat_id'];
			}
			$this->Title = '微群 - 发现微群';
	...
	function members()
	{
		...
		$query = DB::query("SELECT `buddyid`  
							...
							WHERE `uid`='{$uid}'");
		while ($value = DB::fetch($query)) {
			$buddyids[$value['buddyid']] = $value['buddyid'];
		}
		...
		function invite()
	{
		...
		$invite_url = $config['site_url']."/index.php?mod=qun&qid=".$qid;

...
class ModuleObject extends MasterObject
{
	...
	{	
		...
		if (!$backupdir) {			
			...
			$_f_list = (array) $IoHandler->ReadDir(RELATIVE_ROOT_PATH.'backup/db/'.$backupdir,1);
			$f_list = array();
			$key = 0;
			...
			$exportlog = array();		
			if(is_dir(RELATIVE_ROOT_PATH.'backup/'.$backupdir)) {
				$dateline_list = array();
				$key = 0;
				$dir = dir(RELATIVE_ROOT_PATH.'backup/'.$backupdir);
				while($entry = $dir->read()) {
					$entry = RELATIVE_ROOT_PATH.'backup/'.$backupdir.'/'.$entry;
					if(is_file($entry)) {
	...
	function DoImport()
	{
		...
		if($dumpinfo['method'] == 'multivol') {
			...
			shell_exec($mysqlbin.'mysql -h"'.$this->Config['db_host'].($this->Config['db_port'] ? (is_numeric($this->Config['db_port']) ? ' -P'.$this->Config['db_port'] : ' -S"'.$this->Config['db_port'].'"') : '').
			'" -u"'.$this->Config['db_user'].'" -p"'.$this->Config['db_pass'].'" "'.$this->Config['db_name'].'" < '.$datafile);
	...
	function DoDelete()
	{
		...
		if(is_array($delete)) {
			$dir = RELATIVE_ROOT_PATH.'backup/'.$backupdir.'/';

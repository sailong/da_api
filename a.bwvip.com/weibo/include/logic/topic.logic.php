...
$fansurl='home.php?mod=space&do=common&op=topic&code='.$tid.'&uid='.$data['uid'];
		 ...
		 $sql="SELECT  * FROM `" . TABLE_PREFIX ."topic`  WHERE tid= ".$tid;	 
				...
				$topic_list[$value['tid']] =$this->Make($value);
 
			$info = array(
				'list' => $topic_list,
			); 

		foreach($info['list'] as $value){
		$str=$str.$value['content'].'<br>';}
        foreach($value['image_list'] as $img){
		$str=$str."<a href=\"".$fansurl."\" target=_top><img src=\"".$img['image_small']."\" /></a>";
		}   
		
		
		$str=str_replace("index.php?mod=tag&code=","home.php?mod=space&do=common&op=topic&uid=".$data['uid']."&tcode=",$str); 
		
		$nicknamear = $this->DatabaseHandler->FetchFirst("select  nickname from ".DB::table('members')." where uid='{$data['uid']}' ");
  $nickname=$nicknamear['nickname'];
$feed = array(); 
$feed['icon']='topic';
$feed['title_template'] = '<a href="'.$fansurl.'">'.$nickname.'</a> 发布了一条微博'; 
$feed['uid']=$data['uid'];
$feed['username']=$nickname;
$feed['title_data']='<a href="weibo/index.php?mod=topic&code='.$tid.'">'.$subject.'</a>'; 
$feed['body_template'] =$str;
$feed['body_data'] = $feed['body_data'];
$feed['idtype'] = 'topicid';
$feed['id'] = $tid;
$feed['dateline'] =time();
 
  $sql = "insert into `pre_home_feed` (`" . implode("`,`", array_keys
            ($feed)) . "`) values ('" . implode("','", $feed) . "')";
        $this->DatabaseHandler->Query($sql);
        //$tid = $this->DatabaseHandler->Insert_ID();
...
//uc_feed_add($feed['icon'], $data['uid'],  $data['username'], $feed['title_template'], $feed['title_data'], $feed['body_template'], $feed['body_data'], '', '',$feed['images']); 
    ...
    function Delete($ids)
    {
        ...
        foreach ($topics as $topic)
        {
        	...
        	$tid = $topic['tid'];
        	//徐玉枭 同步删除动态
			$this->DatabaseHandler->Query("delete from pre_home_feed where id='{$tid}' and idtype='topicid' and icon='topic'", "SKIP_ERROR");
...
//未修改前 $topic['content'] = preg_replace('~<M ([^>]+?)>\@(.+?)</M>~', '<a href="' . str_replace('|REPLACE_VALUE|', '\\1', $topic_content_member_href_pattern_static) . '"   onmouseover="get_at_user_choose(\'\\2\',\'_user\','.$topic['tid'].',event);"  onmouseout="clear_user_choose();">@\\2</a>', $topic['content']);
//xyx@用户跳转 思路用中转页面uname2uid.php进行跳转 09.17
                        $topic['content'] = preg_replace('~<M ([^>]+?)>\@(.+?)</M>~', '<a href="' .
                str_replace('|REPLACE_VALUE|', '\\1', 'uname2uid.php?username=|REPLACE_VALUE|') .
...
//未修改前 $topic['content'] = preg_replace('~<T>#(.+?)#</T>~e', '"<a href=\"' . str_replace('|REPLACE_VALUE|','" . strip_tags("\\1")', $topic_content_tag_href_pattern_static) . ' . "\">#\\1#</a>"', $topic['content']);
//xyx@用户跳转 思路用中转页面uname2uid.php进行跳转 10.08
$str11='&fuid='.MEMBER_ID;
           $topic['content'] = preg_replace('~<T>#(.+?)#</T>~e', '"<a href=\"' . str_replace('|REPLACE_VALUE|',
                '" . strip_tags("\\1")', $topic_content_tag_href_pattern_static) .'.$str11. "\">#\\1#</a>"', $topic['content']);
                ...
                if($topic['from'])
        {
			...
			//jack add 0328 start
			elseif('iphone'==$topic['from'])
            {
                $topic['from_string'] = "来自大正苹果客户端";
                $topic['from_html'] = '来自大正苹果客户端';
            }
			elseif('ipad'==$topic['from'])
            {
                $topic['from_string'] = "来自大正苹果客户端";
                $topic['from_html'] = '来自大正苹果客户端';
            }
			elseif('android'==$topic['from'])
            {
                $topic['from_string'] = "来自大正安卓客户端";
                $topic['from_html'] = '来自大正安卓客户端';
            }
			//jack add 0328 end
    ...
    function MakeMember($row)
    {
                ...
                if (isset($row['validate']) && $row['validate'])
        {
            ...
            $row['validate_html'] = ($row['validate'] ? "<img class='vipImg' title='{$row['vip_info']}' src='images/vip.gif' />" :
                "");
        }

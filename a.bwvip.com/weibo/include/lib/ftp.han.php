 ...
 *
 * @Filename ftp.han.php $
 *
 * @Author http://www.jishigou.net $
 *
 * @Date 2011-09-09 10:58:28 1320553333 1647614440 6497 $
 *******************************************************************/
...
class FtpHandler
{
	...
	function FtpHandler() {
		$this->set_error(0);
		$this->config = ConfigHandler::get('ftp');
		$this->enabled = false;
		if(empty($this->config['on']) || empty($this->config['host'])) {
			...
			if($this->func == 'ftp_connect' && !function_exists('ftp_connect')) {
				...
				$this->config['host'] = FtpHandler::clear($this->config['host']);
	...
	function ftp_mkdir($directory) {
		$directory = FtpHandler::clear($directory);
	...
	function ftp_put($remote_file, $local_file, $mode = FTP_BINARY) {
		$remote_file = FtpHandler::clear($remote_file);
		$local_file = FtpHandler::clear($local_file);
		$mode = intval($mode);

<?php /* 2013-05-22 in jishigou invalid request template */ if(!defined("IN_JISHIGOU")) exit("invalid request"); ?>
...
<div id="wary">
 ...
 <!--nav-->
 <?php /* 2013-05-21 in jishigou invalid request template */ if(!defined("IN_JISHIGOU")) exit("invalid request"); ?><div id="nav">
	  <div id="nav_one">
		<ul>
			<li>
      ...
      <div id="cen1" style="height:auto;">
         <div id="con1_s"></div>
			<div id="con1_t" style="height:auto;">
				   <div id="t_bok"><?php /* 2013-05-21 in jishigou invalid request template */ if(!defined("IN_JISHIGOU")) exit("invalid request"); ?>
...
<?php } ?></div>
					   <div class="nav_u">
							   <ul>					
									...
									<div class="u2">
											<span><a href="index.php?mod=<?php echo $topiclist['username']; ?>"><?php echo $topiclist['nickname']; ?></a>：<?php echo $topiclist['content']; ?>
<?php if($topiclist['longtextid']) { ?>
 <a href="index.php?mod=topic&amp;code=<?php echo $topiclist['tid']; ?>">[查看全文]</a>
<?php } ?>
</span>
<?php if($topiclist['videoid']) { ?>
											[视频]
											
<?php } ?>
 
<?php if($topiclist['imageid'] && $topiclist['image_list']) { ?>
											<div>
<?php if(is_array($topiclist['image_list'])) { foreach($topiclist['image_list'] as $iv) { ?>
												<p><img src="<?php echo $iv['image_small']; ?>"/></p>
...
<?php } ?>
<div class="t_manger">
												<span class="p_time"><?php echo $topiclist['dateline']; ?>   <?php echo $topiclist['from_string']; ?></span> 
											</div>
											<div>
											 
<?php if($topiclist['uid']==MEMBER_ID || 'admin'==MEMBER_ROLE_TYPE) { ?>
											 <a href="index.php?mod=topic&amp;code=del&amp;tid=<?php echo $topiclist['tid']; ?>">[删除微博]</a> 
...
<?php } ?>
									...
									<div style="padding:2px;">
										<span class="">编辑微博 (<?php echo $this->Config['topic_length']; ?>字以内)</span>
									</div>
									<div class="u2">
<?php if($return_messager) { ?>
<div class="t_tips">微博内容不能为空，请重新编辑</div>
<?php } ?>
<div>
										<p>
										<form id="form1" name="form1" method="post" action="<?php echo $action; ?>">
										 <span>
										 <textarea name="content" id="content" style="width:99%;" rows="3" cols="" /><?php echo $topiclist['content']; ?></textarea>
										 </span>
										<p>
										 <span>
											 <input type="submit" name="Submit" value="编辑" />
											 <input name="tid" type="hidden" id="tid" value="<?php echo $topiclist['tid']; ?>" />
											 <input name="imageid" type="hidden" id="imageid" value="<?php echo $topiclist['imageid']; ?>" />
										 </span>
										</p>
										</form>
										</p>
...
</div><?php /* 2013-05-21 in jishigou invalid request template */ if(!defined("IN_JISHIGOU")) exit("invalid request"); ?><br />

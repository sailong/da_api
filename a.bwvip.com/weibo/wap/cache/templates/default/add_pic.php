...
<div class="t_tips">
<?php if($this->Get['return'] == 'addtopic') { ?>
请输入微博内容
<?php } ?>
</div>
<?php } ?>
<div class="set_warp">
												<div>
													<form action="index.php?mod=topic&amp;code=do_add" method="post" enctype="multipart/form-data" name="form1" id="form1">
													<div>		
														<p>发表微博(<?php echo $this->Config['topic_length']; ?>字以内)</p>
														<span>
<?php $i_already_value = $getvalue ? $getvalue : ''; ?>
<textarea name="content" id="content" style="width:99%;" rows="2" cols="" /><?php echo $i_already_value; ?></textarea>
														</span>
														<p>选择要上传的图片：<input name="topic" type="file" id="topic"/></p>
...
<?php echo array_iconv($this->Config['charset'],'utf-8',renren_syn_html()); ?>
<?php } ?>
<span><p><input type="submit" name="Submit" value="提交" />
														  <input name="topictype" type="hidden" id="topictype" value="first" />
														</p></span>
													</div>
													</form>
												...
												<div>注意：</div>
												<div>1.图片上传速度及大小限制视各地网络情况而定，建议500K以内，如果照片太大，建议压缩后再上传，支持JPG、GIF格式。</div>
												<div>2.目前只支持wap2.0的手机上传，图片文件名请勿包含中文。</div>
											</div>
										</div>
									</div>
							  </ul>

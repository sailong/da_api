...
if($ac=='rank')
{
	...
	if(!empty($event_info))
	{
			...
			while($row = DB::fetch($query))
			{
				...
				{
					$lun_info="";
					$lun_info = DB::fetch_first("select baofen_id,event_id,uid,total_score,score,par from tbl_baofen where event_id='".$event_id."' and uid='".$row['uid']."' and lun='".$j."' ".$fenzhan_sql."  order by dateline asc limit 1 ");
					if($lun_info['score'])
					{

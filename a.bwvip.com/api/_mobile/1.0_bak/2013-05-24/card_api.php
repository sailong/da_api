...
if($ac=='rank')
{
	...
	$event_info=DB::fetch_first("select event_id,event_name,event_uid,event_logo,event_timepic,event_starttime,event_endtime,event_content,event_state,event_is_tj,event_is_baoming,event_addtime from tbl_event where 1=1 ".$sql." order by event_addtime desc limit 1 ");
	if($login_uid)
	{
	...
	if(!empty($event_info))
	{
		...
		if($event_info['event_starttime']>time())
		{
			$response         = 0;
			$error_state      = 0;
			...
			//print_r($data);
			api_json_result(1,0,"该赛事没有开始 no start",$data);
		}
		else
		{
			...
			{
				//如果有分站信息
				$list=DB::query("select uid,realname as username,tlcave as today_score,tlcave as total_score,cave_1,cave_2,cave_3,cave_4,cave_5,cave_6,cave_7,cave_8,cave_9,cave_10,cave_11,cave_12,cave_13,cave_14,cave_15,cave_16,cave_17,cave_18,avcave,(cave_10+cave_11+cave_12+cave_13+cave_14+cave_15+cave_16+cave_17+cave_18) as lin,isend from ". DB::table('golf_nd_baofen')." where fenz_id='".$now_fz_id."' order by isend desc,avcave,lin ");
				while($row=DB::fetch($list))
				{
					$row['id']="0";
					$row['tianshu']="-1";
					...
					$row['today_score']=Getstat($row['today_score']);
...
if($ac=="fenzhan_detail")
{
	...
	if($event_info['event_starttime']<time())
	{
			...
			{
				////分站N洞成绩列表
				$list=DB::query("select uid,realname as username,tlcave as today_score,tlcave as total_score,cave_1,cave_2,cave_3,cave_4,cave_5,cave_6,cave_7,cave_8,cave_9,cave_10,cave_11,cave_12,cave_13,cave_14,cave_15,cave_16,cave_17,cave_18,avcave,(cave_10+cave_11+cave_12+cave_13+cave_14+cave_15+cave_16+cave_17+cave_18) as lin,isend from ". DB::table('golf_nd_baofen')." where fenz_id='".$fz_id."' order by isend desc,avcave,lin ");
				while($row=DB::fetch($list))
				{
					$row['id']="0";
					$row['tianshu']="-1";
			...
			*/
			...
			else
			{
				$data['title']   = "scorerank";
				$data['data'] = null;
				api_json_result(1,1,"比赛没有开始",$data);
			}
		
	}
	else
	{
		api_json_result(1,2,"比赛未开始",$data);
	}

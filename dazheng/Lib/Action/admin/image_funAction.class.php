...
class image_funAction extends AdminAuthAction{
	...
	public function view_img(){
		...
		$this->setImage(600,500,255,255,255,1);//参数(长,高,背影色R,G,B,是否透明1或0)  
		$temparray="0,260,400,124,48,720,122,440,475";//数值,用指定符号隔开  
		$this->setItem(',',$temparray,1,23);//参数(分隔数值的指定符号,数值变量,样式1为竖柱图2为横柱图3为折线图4为饼图,距离)  
		$this->setFont(10);//字体大小1-10  
		$this->setX(1,10);//设置X轴刻度值(起始刻度值=1，刻度间隔值=1)
		$this->PrintReport();
	}

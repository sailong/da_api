 ...
 *    @Date			2013-05-30
 */
class baofen_newAction extends wap_publicAction
{
	...
	public function baofen_login_action()
	{
		...
		if($fenzhan_id==50 || $fenzhan_id==53 || $fenzhan_id==56 || $fenzhan_id==59)
		{
			$_SESSION['fenzhan_ids']="'50,53,56,59'";
		}
		else if($fenzhan_id==51 || $fenzhan_id==54 || $fenzhan_id==57 || $fenzhan_id==60)
		{
			$_SESSION['fenzhan_ids']="'51,54,57,60'";
		}
		else if($fenzhan_id==52 || $fenzhan_id==55 || $fenzhan_id==58 || $fenzhan_id==61)
		{
			$_SESSION['fenzhan_ids']="'52,55,58,61'";
		}
		else
		{
			$_SESSION['fenzhan_ids']=$fenzhan_id;
		}
		...
		if($fenzhan_id)
		{
			...
			{
				$user=M("baofen_user")->where("username='".$username."' and password='".$password."' and fenzhan_id in (".$_SESSION['fenzhan_ids'].") ")->find();
				...
				if($user['baofen_user_id'])
				{
					...
					$this->success("登录成功，现在可以报分了",U('wap/baofen_new/baofen',array('fenzhan_id'=>$fenzhan_id,'fenzu_id'=>$lun)));
				}
				else
				{
					$this->error("用户名或密码有误");
	...
	public function logout()
	{
		if(isset($_SESSION['baofen_user_id']))
		{
			...
			unset($_SESSION['fenzhan_ids']);			

			$this->success("退出成功",U('wap/baofen_new/login',array('fenzhan_id'=>get('fenzhan_id'),'lun'=>get('lun'))));
		}
		else
		{
			$this->error("您已经退出，现在跳转首页",U('wap/baofen_new/login',array('fenzhan_id'=>get('fenzhan_id'),'lun'=>get('lun'))));
		}
	}
	...
	public function baofen()
	{
		...
		{	
			echo "<script>location='".U('wap/baofen_new/login',array('fenzhan_id'=>$fenzhan_id,'lun'=>$lun))."';</script>";
			exit;
		...
		$this->assign('lun',$lun); 
		$this->assign('fenzhan_ids',$_SESSION['fenzhan_ids']); 
		if($fenzhan_id)
		{
			
			$fenzu_list=M()->query("select * from tbl_baofen where fenzhan_id in (".$_SESSION['fenzhan_ids'].") group by fenzu_id order by fenzu_id   ");  
			$this->assign('fenzu_list',$fenzu_list);
			
			//echo "select * from tbl_baofen where fenzhan_id in (".$_SESSION['fenzhan_ids'].") group by fenzu_id order by fenzu_id   ";
			...
			$fenzu_user=M()->query("select * from tbl_baofen where fenzhan_id in (".$_SESSION['fenzhan_ids'].") $strwhere ");  
	...
	{ 
		...
		{ 
			...
			{
				$baofen_info=M()->query("select * from tbl_baofen where baofen_id='".$key."'");
				$fenzhan_info=M("fenzhan")->where(" fenzhan_id='".$baofen_info[0]['fenzhan_id']."' ")->find();
				...
				{
					$parent_fenzhan_id=$baofen_info[0]['fenzhan_id'];
				}
				$sub_arr=array();
				...
				if($event_id && $field_id)
				{
					...
					*/
					...
					//更新总分 0828
					$res=M()->query("update tbl_baofen set total_score=（cave_1+cave_2+cave_3+cave_4+cave_5+cave_6+cave_7+cave_8+cave_9+cave_10+cave_11+cave_12+cave_13+cave_14+cave_15+cave_16+cave_17 +cave_18） where fenzhan_id in (".$_SESSION['fenzhan_ids'].") "); 
			...
			header ( "Location:".U('wap/baofen_new/baofen',array('fenzhan_id'=>$fenzhan_id,'fenzu_id'=>$next_fenzu_id))."" );
		}
		else
		{
			//print_r($_POST);

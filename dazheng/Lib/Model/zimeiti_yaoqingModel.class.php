 ...
 *    @Date			2014-02-13
 */
class zimeiti_yaoqingModel extends Model{

	//list and page
	function zimeiti_yaoqing_list_pro($bigwhere="", $page_size=20, $sort=" zimeiti_yaoqing_addtime desc ") 
	{
		...
		if(get("starttime")!="")
		{
			$where .=" and zimeiti_yaoqing_addtime>".strtotime(get("starttime"))." ";
		}
		if(get("endtime")!="")
		{
			$where .=" and zimeiti_yaoqing_addtime<".strtotime(get("endtime"))." ";
		}
		
		$data["item"]=M("zimeiti_yaoqing")->where($where.$bigwhere)->order($sort)->page($page.",".$page_size)->select();
		...
		$data["total"] = M("zimeiti_yaoqing")->where($where.$bigwhere)->count();
	...
	//nopage select limit 
	function zimeiti_yaoqing_select_pro($bigwhere="",$limit=999999, $sort=" zimeiti_yaoqing_addtime desc ") 
	{
		
		$where = " 1 ";

		$data["item"]=M("zimeiti_yaoqing")->where($where.$bigwhere)->order($sort)->limit($limit)->select();
		//->field("baoming_id,uid,event_id,fenzhan_id,baoming_realname,baoming_mobile,baoming_email,baoming_country,baoming_chadian,baoming_sex,baoming_is_waika,baoming_waika_fenzhan_id,baoming_note,baoming_source,baoming_status,baoming_addtime")
		$data["total"]=M("zimeiti_yaoqing")->where($where.$bigwhere)->count();

<?php
class field_adminModel extends Model{
	...
	function admin_select_all_pro($bigwhere="", $page_size=20, $sort="") 
	{
		...
		$data["item"]=M("field_admin")->where($where.$bigwhere)->order($sort)->page($page.",".$page_size)->select();
		...
		$data["total"] = M("field_admin")->where($where.$bigwhere)->count();
	...
	function admin_select_all_nopage_pro($bigwhere="",$limit=999999, $sort="") 
	{
		...
		$data=M("field_admin")->where($where.$bigwhere)->order($sort)->limit($limit)->select();

...
if($ac=="heng")
{
	...
	while($row=DB::fetch($bf))
	{
		...
		{
			//个人对抗
			$sub_list=DB::query("select baofen_id,sid,event_apply_id,fenzhan_id,uid,realname,start_time,fenzu_id,cave_1,cave_2,cave_3,cave_4,cave_5,cave_6,cave_7,cave_8,cave_9,cave_10,cave_11,cave_12,cave_13,cave_14,cave_15,cave_16,cave_17,cave_18,event_user_id,event_user_team,event_apply_parent_id,fenzu_ing_status from tbl_baofen where 1=1 and fenzu_id='".$row['fenzu_id']."' $big_where order by event_user_team desc  ");
		...
		{
			//4人4球
			$sql_dsfsdf="select baofen_id,sid,event_apply_id,fenzhan_id,uid,realname,start_time,fenzu_id,cave_1,cave_2,cave_3,cave_4,cave_5,cave_6,cave_7,cave_8,cave_9,cave_10,cave_11,cave_12,cave_13,cave_14,cave_15,cave_16,cave_17,cave_18,event_user_id,event_user_team,event_apply_parent_id,fenzu_ing_status from tbl_baofen where 1=1 and fenzu_id='".$row['fenzu_id']."' $big_where group by event_user_team order by event_user_team desc ";
			$sub_list=DB::query($sql_dsfsdf);
		...
		while($row_sub_arr=DB::fetch($sub_list))
		{
			...
			else if($fenzhan_rule==42)
			{
				...
				$user_sub_list_array=array();
				$parent_apply_list=DB::query("select event_apply_id from tbl_baofen where 1=1 and event_apply_id='".$event_apply_info['event_apply_id']."' and fenzhan_id='".$fenzhan_info['fenzhan_id']."' and fenzu_id='".$event_apply_info['fenzu_id']."' ");

				while($parent_apply_list_arr=DB::fetch($parent_apply_list))
				{
					$sub_apply_list=DB::query("select event_apply_realname from tbl_event_apply where 1=1 and parent_id='".$parent_apply_list_arr['event_apply_id']."' and fenzhan_id='".$fenzhan_info['fenzhan_id']."' ");
					while($sub_apply_list_arr=DB::fetch($sub_apply_list))
					{
						$sub_apply_list_array[]=$sub_apply_list_arr['event_apply_realname'];
						$user_sub_list_array[]=$sub_apply_list_arr['event_apply_realname'];
					}
			...
			$row['fenzu_ing_status']=$row_sub_arr['fenzu_ing_status'];
		...
		$fenzu_ing_status=$row['fenzu_ing_status'];
		//如果有成绩，即比赛开始
		$max_cave_1=DB::result_first("select max(cave_1) from tbl_baofen where 1=1 and fenzu_id='".$row['fenzu_id']."' $big_where  ");
		
		//统计UP得分
		for($i=0; $i<18; $i++)
		{
			$d=$i+1;
			$row_sub[0]['up_total']=get_up_44($row_sub[0],$row_sub[1],$d,$fenzu_ing_status,0,'up_total',$max_cave_1);
			$row_sub[1]['up_total']=get_up_44($row_sub[0],$row_sub[1],$d,$fenzu_ing_status,1,'up_total',$max_cave_1);
			
			/*
			echo $row_sub[0]['up_total'];
			echo "<br>";
			echo $row_sub[1]['up_total'];
			echo "<hr>";
			*/
			
			$row_sub[0]['up_'.$d]=get_up_44($row_sub[0],$row_sub[1],$d,$fenzu_ing_status,0,'up_text',$max_cave_1);
			$row_sub[1]['up_'.$d]=get_up_44($row_sub[0],$row_sub[1],$d,$fenzu_ing_status,1,'up_text',$max_cave_1);
		}
		
		$row['win']=ledebei_tongji_text($row_sub[0]['up_total'],$row_sub[1]['up_total'],$max_cave_1,$row['start_time'],$list_info['event_left_ico'],$list_info['event_right_ico'],$row_sub[0]['be_num'],$row_sub[1]['be_num'],$fenzu_ing_status,$type='win');
		$row['status_ico']=ledebei_tongji_text($row_sub[0]['up_total'],$row_sub[1]['up_total'],$max_cave_1,$row['start_time'],$list_info['event_left_ico'],$list_info['event_right_ico'],$row_sub[0]['be_num'],$row_sub[1]['be_num'],$fenzu_ing_status,$type='guoqi');
		$row['status_text']=ledebei_tongji_text($row_sub[0]['up_total'],$row_sub[1]['up_total'],$max_cave_1,$row['start_time'],$list_info['event_left_ico'],$list_info['event_right_ico'],$row_sub[0]['be_num'],$row_sub[1]['be_num'],$fenzu_ing_status,$type='text');
		
		$row_sub[0]['total_text']=ledebei_tongji_text($row_sub[0]['up_total'],$row_sub[1]['up_total'],$max_cave_1,$row['start_time'],$list_info['event_left_ico'],$list_info['event_right_ico'],$row_sub[0]['be_num'],$row_sub[1]['be_num'],$fenzu_ing_status,$type='left_text');
		$row_sub[1]['total_text']=ledebei_tongji_text($row_sub[0]['up_total'],$row_sub[1]['up_total'],$max_cave_1,$row['start_time'],$list_info['event_left_ico'],$list_info['event_right_ico'],$row_sub[0]['be_num'],$row_sub[1]['be_num'],$fenzu_ing_status,$type='right_text');
		
		$row_sub[0]['defen']=ledebei_tongji_text($row_sub[0]['up_total'],$row_sub[1]['up_total'],$max_cave_1,$row['start_time'],$list_info['event_left_ico'],$list_info['event_right_ico'],$row_sub[0]['be_num'],$row_sub[1]['be_num'],$fenzu_ing_status,$type='defen_1');
		$row_sub[1]['defen']=ledebei_tongji_text($row_sub[0]['up_total'],$row_sub[1]['up_total'],$max_cave_1,$row['start_time'],$list_info['event_left_ico'],$list_info['event_right_ico'],$row_sub[0]['be_num'],$row_sub[1]['be_num'],$fenzu_ing_status,$type='defen_2');

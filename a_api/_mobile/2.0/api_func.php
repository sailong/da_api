...
function ledebei_tongji_text($up_total_left,$up_total_right,$max_cave,$start_time,$guoqi_left,$guoqi_right,$be_num_left,$be_num_right,$fenzu_ing_status,$type='guoqi')
{

	$left_text='';
	$right_text='';
		
	if($max_cave>0 && $max_cave<100)
	{
		//
		if($up_total_left==$up_total_right)
		{
			$text="AS";
			$win="";
			
			if($fenzu_ing_status==1)
			{
				$defen_1=0.5;
				$defen_2=0.5;
			}
		...
		else
		{
			if($up_total_left>$up_total_right)
			{
				$up_total=($up_total_left-$up_total_right);
				$guoqi=$guoqi_left;
			...
			else if($up_total_left<$up_total_right)
			{
				$up_total=($up_total_right-$up_total_left);
				$guoqi=$guoqi_right;
				$be_num=$be_num_right;
...
function get_up_44($arr1,$arr2,$dong_number=0,$fenzu_ing_status=0,$team_number=0,$type='up_text',$max_cave_1)
{
	/*
	print_r($arr1);
	echo "<hr>";
	...
	*/
	...
	for($i=0; $i<18; $i++)
	{
		...
		if($score1==0 && $score2==0 &$up_value[$i]>0)
		{
			$up1_text[$i]='';
			$up2_text[$i]='';
		}

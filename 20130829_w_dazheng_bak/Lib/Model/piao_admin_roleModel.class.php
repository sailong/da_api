<?php
class piao_admin_roleModel extends Model
{
	...
	function admin_role_select_all_pro($bigwhere="", $page_size=20, $sort="") 
	{
		...
		$data["item"]=M("piao_admin_role")->where($where.$bigwhere)->order($sort)->page($page.",".$page_size)->select();
		
		for($i=0; $i<count($data["item"]); $i++)
		{

		}
		$data["total"] = M("piao_admin_role")->where($where.$bigwhere)->count();
	...
	function admin_role_select_all_nopage_pro($bigwhere="",$limit=999999, $sort="") 
	{
		...
		$data=M("piao_admin_role")->where($where.$bigwhere)->order($sort)->limit($limit)->select();

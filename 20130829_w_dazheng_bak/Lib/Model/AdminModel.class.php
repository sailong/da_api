...
class AdminModel extends Model{
	...
	function admin_select_all_pro($bigwhere="", $page_size=20, $sort="") 
	{
		...
		$data["item"]=M("admin")->where($where.$bigwhere)->order($sort)->page($page.",".$page_size)->select();
		for($i=0; $i<count($data["item"]); $i++)
		{
			if($data["item"][$i]["admin_role_id"]!="")
			{
				$user=M()->query("select admin_role_name from tbl_admin_role where  admin_role_id='".$data["item"][$i]["admin_role_id"]."' ");
				$data["item"][$i]["admin_role_name"]=$user[0]["admin_role_name"];
			}
		}
		$data["total"] = M("admin")->where($where.$bigwhere)->count();
	...
	function admin_select_all_nopage_pro($bigwhere="",$limit=999999, $sort="") 
	{
		...
		$data=M("admin")->where($where.$bigwhere)->order($sort)->limit($limit)->select();

...
function checkDropDown(obj,flag){
	if(flag){
		$.each( $("input[name="+obj+"]").parent().find("table td"), function(i, n){
			if($(n).attr("bgColor") == "#003399"){
				$(n).attr("bgColor","#ff0000");
			}
		});		
		$.each( $("input[name="+obj+"]").parent().find("table td img"), function(i, n){
		  if($(n).attr("src").indexOf("003399.gif") > -1){
			$(n).attr("src",$(n).attr("src").replace("003399.gif","ff0000.gif"));
		  }else{
		...
		$.each( $("input[name="+obj+"]").parent().find("table td"), function(i, n){
			if($(n).attr("bgColor") == "#ff0000"){
				$(n).attr("bgColor","#003399");
			}
		});		
		$.each( $("input[name="+obj+"]").parent().find("table td img"), function(i, n){
		  if($(n).attr("src").indexOf("ff0000.gif") > -1){
			$(n).attr("src",$(n).attr("src").replace("ff0000.gif","003399.gif"));
		  }else{
...
function submitForm(){
	...
	*/
	...
	if(flag){
		...
		$.ajax({
		   type: "POST",
		   url: "/cms/cn/zh/events/golf_ticket_2013/service.jsp",
		   data: "gender="+gender+"&family_name="+$("input[name=family_name]").val()+"&given_name="+$("input[name=given_name]").val()+"&birthday="+birthday+"&mobile="+$("input[name=mobile]").val()+"&email="+$("input[name=email]").val()+"&province="+$("input[name=province]").val()+"&city="+$("input[name=city]").val()+"&address="+$("textarea[name=address]").val()+"&post_code="+$("input[name=post_code]").val()+"&ticket_date="+ticket_date+"&ticket="+$("input[name=ticket]").val()+"&bmw_owner="+$("input[name=bmw_owner]").val()+"&intended_series="+$("input[name=intended_series]").val()+"&intended_date="+$("input[name=intended_date]").val()+"&dealer_contact_usage="+$("input[name=dealer_contact_usage]").val()+"&data_usage="+$("input[name=data_usage]").val()+"&information_source="+information_source_val,
		   success: function(msg){
			 $("#washing").hide();
			// $("#controllor").attr("src","about:blank").attr("src",controllorUrl);
			  if(msg.indexOf("successful.html") == 0){
				//document.location="successful.html?controllor_url="+controllorUrl;
				document.location="successful.html";
			 }else if(msg.indexOf("same_data.html") == 0){
				$(".below_dotted_line_left").css("display","inline");
			 	$("#common_phone_section_error").html("请检查您的输入内容，或者该E-mail地址是否已经被注册过。").show();
			 	$("#email_form_item").addClass("error");
			 }else{			 	

...
if($ac=="push_msg_list")
{
	...
	$list=DB::query("select message_id,message_number,message_type,uid,message_content,message_addtime from tbl_push_message where uid='$uid' or uid=0 order by message_addtime desc ");
	...
	while($row=DB::fetch($list))
	{
		...
		//$row['message_content']=json_decode($row['message_content'],true);

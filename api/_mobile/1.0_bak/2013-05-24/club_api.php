...
if($ac=="club_index_nologin")
{
	...
	if($max_page>=$page)
	{
			...
			$list=DB::query("select tid,uid,roottid,(select realname from ".DB::table("common_member_profile")." where uid=jishigou_topic.uid) as username,content,replys,forwards,dateline,(select photo from jishigou_topic_image where tid=jishigou_topic.tid limit 1) as photo,voice,voice_timelong from jishigou_topic type<>'reply' order by dateline desc limit $page_start,$page_size ");
			while($row = DB::fetch($list) )
			{
				...
				$root_topic=DB::fetch_first("select tid,uid,(select realname from ".DB::table("common_member_profile")." where uid=jishigou_topic.uid) as username,content,replys,forwards,dateline,(select photo from jishigou_topic_image where tid=jishigou_topic.tid limit 1) as photo,voice,voice_timelong from jishigou_topic where tid='".$row['roottid']."' order by dateline asc ");
				if($root_topic)
				{
					if($root_topic['photo'])
					{
...
if($ac=="club_index_login")
{
	...
	if($max_page>=$page)
	{
		...
		$list=DB::query("select tid,uid,roottid,(select realname from ".DB::table("common_member_profile")." where uid=jishigou_topic.uid) as username,content,replys,forwards,dateline,(select photo from jishigou_topic_image where tid=jishigou_topic.tid limit 1) as photo,voice,voice_timelong from jishigou_topic  where type<>'reply' and uid in (select buddyid from jishigou_buddys where uid='".$uid."' )  order by dateline desc limit $page_start,$page_size ");
		while($row = DB::fetch($list) )
		{
			if($row['photo'])
			{
...
(select realname from ".DB::table("common_member_profile")." where uid=jishigou_topic.uid) as username,content,replys,forwards,dateline,(select photo from jishigou_topic_image where tid=jishigou_topic.tid limit 1) as photo,voice,voice_timelong from jishigou_topic where tid='".$tid."' ");
...
(select realname from ".DB::table("common_member_profile")." where uid=jishigou_topic.uid) as username,content,replys,forwards,dateline,type,(select photo from jishigou_topic_image where tid=jishigou_topic.tid limit 1) as photo from jishigou_topic where totid='".$tid."'  order by dateline desc limit 10 ");
		...
		//根topic
		$root_topic=DB::fetch_first("select tid,uid,(select realname from ".DB::table("common_member_profile")." where uid=jishigou_topic.uid) as username,content,replys,forwards,dateline,(select photo from jishigou_topic_image where tid=jishigou_topic.tid limit 1) as photo,voice,voice_timelong from jishigou_topic where tid='".$detail_data['roottid']."' order by dateline asc ");
		if($root_topic)
		{
			if($root_topic['photo'])
			{
...
if($ac=="member_detail")
{
	...
	if($get_uid)
	{
		...
		if($max_page>=$page)
		{
			$list=DB::query("select tid,uid,roottid,(select realname from ".DB::table("common_member_profile")." where uid=jishigou_topic.uid) as username,content,replys,forwards,dateline,(select photo from jishigou_topic_image where tid=jishigou_topic.tid limit 1) as photo,voice,voice_timelong from jishigou_topic where  uid='".$get_uid."' order by dateline desc limit $page_start,$page_size ");
			while($row = DB::fetch($list) )
			{
				if($row['photo'])
				{
	...
	if($uid)
	{
		...
		else
		{
			...
			if($max_page>=$page)
			{
				...
				$list=DB::query("select tid,uid,roottid,(select realname from ".DB::table("common_member_profile")." where uid=jishigou_topic.uid) as username,content,replys,forwards,dateline,(select photo from jishigou_topic_image where tid=jishigou_topic.tid limit 1) as photo,voice,voice_timelong from jishigou_topic where type<>'reply' and content like '%<M ".$username.">%' order by dateline desc limit $page_start,$page_size ");
				while($row = DB::fetch($list) )
				{
					if($row['photo'])
					{
	...
	if($uid)
	{
		...
		else
		{
			...
			if($max_page>=$page)
			{
					...
					$list=DB::query("select tid,uid,(select realname from ".DB::table("common_member_profile")." where uid=jishigou_topic.uid) as username,content,replys,forwards,dateline,(select photo from jishigou_topic_image where tid=jishigou_topic.tid limit 1) as photo,voice,voice_timelong from jishigou_topic where type<>'reply' and totid='".$uid."' and type='reply' order by dateline desc limit $page_start,$page_size ");
					while($row = DB::fetch($list) )
					{
						if($row['photo'])
						{
...
if($ac=="app_version")
{
	...
	$version =DB::fetch_first( " select app_version_type,app_version_number,app_version_name,app_version_content,app_version_file,app_version_is_important,app_version_addtime from tbl_app_version where app_version_type ='".$app_version_type."' order by app_version_addtime desc limit 1  ");

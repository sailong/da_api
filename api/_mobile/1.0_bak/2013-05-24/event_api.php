...
if($ac=="select_event")
{
	$list3=DB::query("select event_id,event_name,event_uid,event_is_zhutui,event_zhutui_pic,event_content from tbl_event where event_is_zhutui='Y' order by event_sort desc limit 1 ");
	...
	$list=DB::query("select event_id,event_name,event_uid,event_is_zhutui,event_content from tbl_event where event_is_tj='Y' order by event_sort desc limit 100 ");
	...
	$list2=DB::query("select event_id,event_name,event_uid,event_is_zhutui,event_content from tbl_event where event_baoming_starttime<=".time()." and event_baoming_endtime>=".time()." order by event_sort desc  limit 100 ");
...
if($ac=="apply_ing")
{
	...
	$list=DB::query("select event_id,event_name,event_uid,event_is_zhutui,event_content from tbl_event where event_baoming_starttime<=".time()." and event_baoming_endtime>=".time()." order by event_baoming_starttime desc  limit 100 ");
	while($row = DB::fetch($list))
	{
		if($login_uid)
		{
...
if($ac=="event_blog_detail")
{
	...
	if($blogid)
	{
		...
		$list=DB::query("select arc_id as blogid,arc_name as subject,arc_addtime as dateline from tbl_arc order by arc_addtime desc limit 2");
		while($row = DB::fetch($list))
		{
			if($row['dateline'])
			{
	...
	if($event_id || $sid)
	{
		...
		if($detail_data)
		{
			...
			if($max_page>=$page)
			{
				...
				$list=DB::query("select tid,uid,roottid,(select realname from ".DB::table("common_member_profile")." where uid=jishigou_topic.uid) as username,content,replys,forwards,dateline,(select photo from jishigou_topic_image where tid=jishigou_topic.tid limit 1) as photo,voice,voice_timelong from jishigou_topic where type<>'reply' and content like '%".$tag."%' order by dateline desc limit $page_start,$page_size ");
				while($row = DB::fetch($list) )
				{
					if($row['photo'])
					{
...
if($ac=="rule")
{
		...
		if($detail_data)
		{
			...
			if($max_page>=$page)
			{
				...
				$tag=$detail_data['event_name'];
				$list=DB::query("select tid,uid,roottid,(select realname from ".DB::table("common_member_profile")." where uid=jishigou_topic.uid) as username,content,replys,forwards,dateline,(select photo from jishigou_topic_image where tid=jishigou_topic.tid limit 1) as photo,voice,voice_timelong from jishigou_topic where type<>'reply' and fuid='1899466' order by dateline desc limit ".$page_start.",".$page_size." ");
				while($row = DB::fetch($list) )
				{
					if($row['photo'])
					{
